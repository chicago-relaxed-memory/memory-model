...
\begin{align*}
  ...
  {\rhb} &= ({\rpox} \cup {\rsw})^+
\end{align*}
Note that for top-level pomsets, our semantics guarantees that
${\rsw}\subseteq{\le}$.
...
\begin{definition}
  Let $(\SC \guard \aPSS)$ be the subset of $\aPSS$ such that $\aPS\in\aPSS$
  whenever $\aPS$ is a top-level pomset and ${\lt}\cup{\rpox} \cup {\reco}$
  is acyclic.
  Let $\semsc{\aCmd} = \SC\guard\sem{\aCmd}$. 
\end{definition}
%\begin{itemize}
...
We only consider \emph{generators}, which are pomsets that are minimal with
respect to augmentation and implication.  Since we are considering finite
programs without loops, the pomsets in the semantics of threads are finite.
Thus, there are no infinite descending chains of augmentations.
