...
Since we are considering finite programs without loops, the pomsets in the semantics of threads are finite.  Thus, there are no infinite descending chains of augmentations.  We will only consider the minimal elements.  In these minimal elements $\aPS \in\semClosed{\aCmd}$, $\gtN$ is the transitive closure of $(\le \cup \reco)$ and furthermore they satisfy the following properties.   We write the condition for the case $n=2$ for notational convenience.
...
Let $\aPS$ in $\aCmd$, as per remark~\ref{generator}.   The following theorem is proved in Appendix~\ref{drfproof}. 
