...
Let us revisit the discussion of the us of \!$\PAR$\!
...
\item[5f.] if $\labelingAct(\bEv)=\DFS{\modeREL}$ and $\aEv$ writes, then
  $\bEv \lt' \aEv$, and
\item[6.] if $\bEv$ is an acquire then $\labelingForm(\aEv)$ is location
  independent.
\end{enumerate}
...
\item[5f.] if $\labelingAct(\bEv)=\DFS{\modeREL}$ and $\aEv$ writes, then
  $\bEv \ORDER \aEv$, and
\item[6.] \textcolor{red}{if $\bEv$ is an acquire then $\labelingForm(\aEv)$ is location
  independent.}
% \item 
%   if
...
NOTE: in defining equivalence, we drop incompatible worlds.
