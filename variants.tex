...
Pomsets have at most one substitution. Substitutions track dependencies for
the follower.  There may not be any events following a substitution.
...
\begin{definition}
  \label{def:semi:seq}
  Let $\aPS'\in (\aPSS^1 \sequence \aPSS^2)$ %be the set $\PRE(\aPSS')$ where $\aPS'\in\aPSS'$ 
