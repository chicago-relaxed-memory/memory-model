...
We assume that each register is assigned at most
once.  To keep the presentation as simple as possible, we include neither
...
\item[5a.] if $\aEv$ writes %, there is some $\dEvs\in\READS(\aPS^1)$ such that when $\dEv\in\dEvs$, 
  ...
  $\SUBPAR{\SUBDRS{\dEvs}}{\aReg/\aReg} \SUBAPP (\aSub\SUBAPP\labelingForm^2(\aEv))$,
% \item[----]  
%  for every $\dEvs\in\drs(\aPS^1)$, either
%   \begin{itemize}
%   \item[4c]
