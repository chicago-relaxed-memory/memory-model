...
\todo{The rewrites work.}  The extension is simple: For address expressions
$\REF{\aExp}$ and $\REF{\bExp}$, replace $\aLoc=\bLoc$ by provable equality
of $\aExp$ and $\bExp$, and $\aLoc\neq\bLoc$ by provable inequality.
Operations on sets can be defined similarly.
...
%   % \\[6ex]
% \end{align*}
\todo{We encode address calculation as conditional execution.  For example,
  Candidate \ref{cand:ord} becomes the following, and similarly for the
  definitions in \textsection\ref{sec:refine}.}
  \begin{align*}
  \sem{\aReg\GETS\REF{\cExp}^\amode\SEMI \aCmd} & =
...
\begin{align*}
  ...
  \hbox{\begin{tikzinline}[node distance=1em]
    ...
    \end{tikzinline}}%\;\;\cdots
\end{align*}
\todo{talk about how these pomsets come up}
...
\begin{gather}
  ...
  \hbox{\begin{tikzinline}[node distance=1em]
      \event{a}{r\EQ1\mathbin{\mid}\DW{[1]}{0}}{}
      \event{b}{r\EQ0\lor r\EQ1\mathbin{\mid}\DW{[0]}{0}}{right=of a}
      %\co{a}{b}
      %\event{c}{r\EQ1\mathbin{\mid}\DW{[1]}{0}}{right=of b}
    \end{tikzinline}}%\;\;\cdots
\end{gather}
