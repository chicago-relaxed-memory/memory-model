...
\begin{definition}
  \label{def:semi:seq}
  Let $\bEvs$ be a \emph{disjoint read set} of $\aPS$  when 
  (1) $\bEvs\subseteq\Event$,
  (2) all $\aEv\in\bEvs$ are reads,
  (3) if $\aEv\in\Event$ reads from $\aLoc$ then some $\bEv\in\bEvs$ reads from $\aLoc$, and
  (4) if $\aEv\in\bEvs$ reads from $\aLoc$ then
  (4a) if $\bEv\in\bEvs$ reads from $\aLoc$ then $\bEv=\aEv$, and
  (4b) if $\bEv\in\Event$ reads from $\aLoc$ then $\aEv\not\le\bEv$.

  Let $\drs(\aPS)$ be the set of disjoint read sets of $\aPS$.

  For $\dEvs\in\drs(\aPS)$, let $\SUBDRS{\dEvs}$ be the substitution such
  ...
  $\Event^2$.  For each $\aEv$, let $B_\aEv\subseteq\Event^1$ be a set of
...
\item[4bc.] $\labelingForm'(\aEv)$ implies $\SUBDRS{\dEvs} \SUBAPP
  (\labelingAct^1(\cEv)\SUBAPP\labelingForm^2(\aEv))$, for $\dEvs\in\drs(\aPS^1)$,
% \item[4b.] if $\bEv$ \externally reads $\aVal$ from $\aLoc$ then
%   $\labelingForm'(\aEv)$ implies $\labelingForm^2(\aEv)[\aVal/\aLoc]$,
% \item[4c.] if $\bEv$ does not \externally read then $\labelingForm'(\aEv)$
%   implies $\labelingForm^2(\aEv)$,
\item[5a.] if $\labelingForm'(\aEv)$ does not imply $\labelingAct^1(\cEv)\SUBAPP\labelingForm^2(\aEv)$ and
  $\aEv$ writes, then for some $\dEvs\in\drs(\aPS^1)$ and every $\dEv\in \dEvs$, $\dEv\ORDER\aEv$, 
\item[5b-f.] as before,
% \item[5b.] if $\bEv$ and $\aEv$ are \external actions in conflict, then
%   $\bEv\ORDER\aEv$,
% \item[5c.] if $\bEv$ is an acquire or $\aEv$ is a release, then
