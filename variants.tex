...
\begin{align*}
  \sem{\SKIP} & \eqdef
  \PREPAR{\TIKZ{\final{f}{\SUBEMP}{}}}
  \\  
  \sem{\aReg\GETS\aExp} & \eqdef
  \PREPAR{\TIKZ{\final{f}{\SUB{\aExp/\aReg}}{}}}
  \\
  \sem{\FENCE^{\fmode}} & =
  \PREPAR{\TIKZ{
      \event{a}{\DFS{\fmode}}{}
...
\begin{definition}
  \label{def:semi:seq}
  $\DISJUNCT(\aPSS)$ be the set $\DISJUNCT(\aPSS')$ where $\aPS'\in\aPSS'$ if
  there is $\aPS\in\aPSS$ such that $\Event' = \Event$,
  ${\le'}\supseteq{\le}$, and for every $\aEv\in\Event$ there are
  $\aPS^i\in\aPSS$ that are identical but for the precondition of $\aEv$ such
  that $\labelingForm'(\aEv)$ implies $\bigvee_i \labelingForm^i(\aEv)$.  
  
  Let $\aPS'\in (\aPSS^1 \sequence \aPSS^2)$ be the set $\DISJUNCT(\aPSS')$ where $\aPS'\in\aPSS'$ 
  when either (a) $\aPS' \in \aPSS^1$ and
  $\disjoint{\Event'}{\Sub}$, or (b) there are $\aPS^1 \in \aPSS^1$,
  $\cEv\in\Event^1\cap\Sub$ and $\aPS^2\in\aPSS^2$ such that the following hold.  Let
...
\item[6b.] if $\labelingAct(\bEv)=\DFW[\aLoc]{\modeREL}$, $\aEv$ is a release
  that does not write $\aLoc$, and $\bEv\le\aEv$, then there is some
  $\bEv\le'\dEv\le'\aEv$ such that $\bEv$ (explicitly) writes
  $\aLoc$.  % For $\amode\neq\modeRLX$, let
\end{enumerate}
