...
\item $\sem{\aReg\GETS\aExp}$ introduces $[\aExp/\aReg]$,
\item $\sem{\aReg\GETS\aLoc}$, local rule, introduces $[\aLoc/\aReg]$,
\item $\sem{\aReg\GETS\aLoc}$, nonlocal rule, introduces
  $(\DR{\aLoc}{\aVal}) \le [\aVal/\aReg]$.
\end{itemize}
% Base cases:
% \begin{align*}
%   \sem{\SKIP}
...
\begin{definition}
  \label{def:semi:seq}
  Let $\aPS' \in (\aPSS^1 \sequence \aPSS^2)$ whenever (a)
  $\aPS' \in \aPSS^1$ and $\disjoint{\Event'}{\Sub}$ or (b) there are
  $\aPS^1 \in \aPSS^1$, $\cEv\in\Event^1$ and $\aPS^2\in\aPSS^2$ such that
  the following hold.  Let $\bEv$ range over $\Event^1\setminus\Sub$, and
  $\aEv$ range over $\Event^2$.
\begin{enumerate}
...
\item[4a2.] $\labelingAct'(\aEv) =  \labelingAct^2(\aEv);\labelingAct^1(\cEv)$,  for $\aEv\in\Event^2\cap\Sub$, 
...
\item[6.] if $\bEv$ is a release, $\dEv\in\Event'$ is an acquire,
  $\dEv\le'\aEv$, then $\labelingForm(\aEv)$ is location independent.
\end{enumerate}
\begin{gather*}
  \sem{\aCmd \SEQ \bCmd}
