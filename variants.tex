...
Substitutions track dependencies.  To give a sensible definition for
concurrency, we must break the asymmetry of \!$\PAR$\!.  Arbitrarily, we keep
the state of the thread on the left.
...
\item $\sem{\aReg\GETS\aLoc}$ introduces the substitution $[\aLoc/\aReg]$,
  ordered after any new read action (but not a silent action).
...
\begin{definition}
  ...
  $\aPS^1 \in \aPSS^1$ and
  $\{\aPS^{\cEv}\in\aPSS^2 \mid \cEv\in\Event^1\cap\Sub\}$, such that the
  following hold.  We use several conventions:
  Let $\aSub_{\cEv}=\labelingAct^1(\cEv)$.
  Let $\bEv\in\killS(\aPS^1)$.
  Let $\aEv\in\biguplus_{\cEv}\Event^{\cEv}$.
  Let $\bEv\ORDER\aEv$ if either $\bEv \lt' \aEv$ or $\aEv\in\Event^{\cEv}$
  and $\bEv\not\le^1\cEv$. %, for some $\cEv$.
\begin{enumerate}
\item[1.] $\Event' = \killS(\aPS^1) \uplus \biguplus_{\cEv}\Event^{\cEv}$,
\item[2.] ${\le'}\supseteq{\le^1}\cup\bigcup_{\cEv}{\le^{\cEv}}$, 
\item[3a.] $\labelingAct'(\bEv) = \labelingAct^1(\bEv)$,
\item[3b.] $\labelingForm'(\bEv)$ implies $\labelingForm^1(\bEv)$,
\item[4a.] $\labelingAct'(\aEv) = \labelingAct^{\cEv}(\aEv)$,
\item[4b.] if $\bEv$ \externally reads $\aVal$ from $\aLoc$ then
  $\labelingForm'(\aEv)$ implies $\labelingForm^{\cEv}(\aEv)[\aVal/\aLoc]$,
\item[4c.] if $\bEv$ does not \externally read then $\labelingForm'(\aEv)$
  implies $\labelingForm^{\cEv}(\aEv)$,
\item[5a.] if $\labelingForm'(\aEv)$ does not imply $\labelingForm^{\cEv}(\aEv)$ and
  $\aEv$ writes, then $\bEv\ORDER\aEv$,
\item[5b.] if $\bEv$ and $\aEv$ are \external actions in conflict, then
  $\bEv\ORDER\aEv$,
\item[5c.] if $\bEv$ is an acquire or $\aEv$ is a release, then
  $\bEv \ORDER \aEv$,
...
\item 
  ...
  $\labeling^{\cEv}(\aEv) = (\bForm \mid \bActSub)$ then
