...
We assume that each register is assigned at most once\nofootnote{One can remove
  this restriction by defining conflict (Definition~\ref{def:prefix}) to
  include actions that read into the same register; this includes order
  between reads in item 5b.  In addition, let $\READS(\aPS)=\{\aEv\mid\aEv$
  is a read into $\aReg$ and there is no $\aEv$ that reads into $\aReg$ such
  that $\dEv\le\aEv\}$.  For Theorem~\ref{thm:seq} to hold, register conflict
  must then be included in $\semold{}$.} and refactor the syntax:
\begin{align*}
  \aCmd,\,\bCmd
  \BNFDEF& \SKIP
  \BNFSEP \FENCE^{\fmode}
...
\begin{align*}
  \aLocReg,\,\bLocReg\!\!\BNFDEF\!\! \aLoc \!\BNFSEP\! \aReg
  &&
  \aSub,\,\bSub,\,\beforeSub,\,\afterSub\!\!\BNFDEF\!\! \SUBEMP \!\BNFSEP\! \SUBPAR{\aSub}{\aEExp/\aLocReg}
...
base case says
\begin{math}
  \SUBPAR{\aSub}{\aEExp/\aLocReg}\SUBAPP\bLocReg
\end{math}
is $\aEExp$ if $\aLocReg=\bLocReg$ and $\aSub \SUBAPP\bLocReg$ otherwise.
We write $\SUBPAR{\SUBEMP}{\aEExp/\aLocReg}$ as $\SUB{\aEExp/\aLocReg}$ and 
$\SUBPAR{\SUBPAR{\aSub}{\bEExp/\aLocReg}}{\aEExp/\aLocReg}$ as
$\SUBPAR{\aSub}{\aEExp/\aLocReg}$.
...
$\aSub_\cEv=\labelingAct(\cEv)$,
and
$\labelingAct(\aEv)=\DRreg[\amode]{\aReg}{\beforeSub}{\afterSub}{\aLoc}{\aVal}$, 
then
$\beforeSub\SUBSEQ\afterSub = \aSub_\cEv$.

For $\dEvs\subseteq \READS(\aPS)$, let $\SUBDRS{\dEvs}$ be the derived
substitution of values for registers: $\SUBDRS{\dEvs}\SUBAPP\aReg$ is $\aVal$
if some $\dEv\in\SUBDRS{\dEvs}$ reads $\aVal$ into $\aReg$, and is $\aReg$
otherwise.
...
\item[1.] $\Event' = \Event^1\setminus\Sub \uplus \Event^2$,
\item[2.] ${\le'}\supseteq{\le^1}\cup{\le^2}$, 
\item[3a.] $\labelingAct'(\bEv) = \labelingAct^1(\bEv)$ (update $\afterSub$ for reads...) %, for $\bEv\in\Event^1\setminus\READS(\aPS^1)$,
...
%   if
%   $\labelingAct^1(\bEv)
%   =\DRreg[\amode]{\aReg}{\beforeSub}{\afterSub}{\aLoc}{\aVal}$, where if
%   $\aEv\in\Event^2\cap\Sub$ then $\aSub=\labelingAct^2(\aEv)$ otherwise $\aSub=\SUBEMP$,
\item[3b.] $\labelingForm'(\bEv)$ implies $\labelingForm^1(\bEv)$,
\item[4a1.] $\labelingAct'(\aEv) = \labelingAct^2(\aEv)$,  for $\aEv\in\Event^2\setminus\Sub$, (update $\beforeSub$ for reads...)
\item[4a2.] $\labelingAct'(\aEv) =  \aSub_\cEv\SUBSEQ\labelingAct^2(\aEv)$,  for $\aEv\in\Event^2\cap\Sub$, 
\item[4bc.] $\labelingForm'(\aEv)$ implies
  $(\SUBDRS{\dEvs} \SUBSEQ \aSub_\cEv)\SUBAPP\labelingForm^2(\aEv)$, 
\item[5a.] if $\aEv$ writes and
  $\bEv=\DRreg[\amode]{\aReg}{\beforeSub}{\afterSub}{\aLoc}{\aVal}$
  then either $\bEv\le'\aEv$ or
  $\labelingForm'(\aEv)$ implies  
  $(\SUBDRS{\dEvs}\SUBSEQ \beforeSub \SUBSEQ \SUB{\aLoc/\aReg} \SUBSEQ \afterSub)\SUBAPP\labelingForm^2(\aEv)$,
...
%   $\SUBPAR{\SUBDRS{\dEvs}}{\aLoc/\aReg} \SUBAPP (\aSub_\cEv\SUBAPP\labelingForm^2(\aEv))$,
\item[5b-f.] as before,
\item[6a.] if $\bEv$ is a release, $\dEv\in\Event'$ is an acquire,
  $\bEv\le'\dEv\le'\aEv$, then $\labelingForm(\aEv)$ is location independent, and
\item[6b.] if $\labelingAct(\bEv)=\DFW[\aLoc]{\amode}$, $\bEv\le\aEv$, and
...
\begin{gather*}
  ...
  \begin{gathered}
    ...
    \\[-1ex]
    \hbox{\begin{tikzinline}[node distance=1em]
      \event{c}{r{=}s\mid\DW{y}{1}}{right=of b}
      \end{tikzinline}}
  \end{gathered}
\end{gather*}

The see the need for substitution decomposition on reads in 5a, consider:
\begin{gather*}
  \begin{gathered}
