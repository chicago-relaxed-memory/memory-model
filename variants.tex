...
\begin{enumerate}
\item[5e.] if $\cEv$ reads and $\labelingAct(\aEv)=\DFS{\modeACQ}$, then
  $\aEv \lt' \cEv$,
\item[5f.] if $\labelingAct(\cEv)=\DFS{\modeREL}$ and $\aEv$ writes, then
  $\cEv \lt' \aEv$,
\end{enumerate}
...
\begin{itemize}
\item if $c$ is a write that conflicts with $e$, $c\gtN e$ and $d \xrmw e$ then  $c\gtN d$, and
\item if $c$ is a write that conflicts with $e$, $d\gtN c$ and $d \xrmw e$ then  $e\gtN c$.
\end{itemize}
...
Substitution actions represent a final state of registers.  To give a
sensible definition for concurrency, we must break the asymmetry of \!$\PAR$\!.
Arbitrarily, we choose to keep the state of the thread on the left.
\begin{definition}
