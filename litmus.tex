...
\cite{DBLP:journals/pacmpl/PulteFDFSS18}.  In this section, we walk through
several litmus tests, valid rewrites and invalid rewrites.  The examples show
that rules \ref{5a}-\ref{5d} are understandable as \emph{general principles}, not
simply the intersection of a series of relations.
...
problem with Javascript \cite[\textsection27]{ecma2019}:
\begin{gather*}
  \taglabel{SC2}
  \begin{gathered}
...
In Javascript, the rules relating SC and relaxed access are subtle.  As
...
\begin{gather*}
  ...
  \hbox{\begin{tikzinline}[node distance=1.5em]
      ...
      \sync{a1}{a2}
      \event{b1}{\DW{x}{2}}{right=2em of a2}
      \event{b2}{\DW[\modeRA]{\,z}{1}}{right=of b1}
      \sync{b1}{b2}
      \event{c1}{\DR[\modeRA]{\,z}{1}}{right=2em of b2}
      ...
      \rf[out=12,in=168]{a2}{c2}
      \rf{b2}{c1}
      \wk[out=19,in=161]{a1}{b1}
      \wk[out=-169,in=-11]{c4}{b1}
    \end{tikzinline}}
\end{gathered}
\end{gather*}
% \begin{gather*}
...
If this outcome were allowed, then data racing writes would be visible, even
after a full synchronization; this would invalidate local reasoning about
data races (\textsection\ref{sec:sc}).
...
cost of introduce the concept of \emph{dead reads}.
...
first is imposed by \ref{5a}, the second by \ref{4b}, ensuring that the two
reads see the same value.

These executions are  examples of \emph{internal reads}, using \armeight{}
terminology \cite{DBLP:journals/pacmpl/PulteFDFSS18}.
...
\begin{gather*}
  ...
  \sync{a1}{a2}
  \event{b3}{\DRAcq{a}{1}}{below right=of a2}
  \rf{a2}{b3}
  \event{b4}{\DW{x}{2}}{right=of b3}
  \sync{b3}{b4}
  \event{b5}{\DWRel{b}{1}}{right=of b4}
  \sync{b4}{b5}
  \event{a6}{\DRAcq{b}{1}}{above right=of b5}
  \rf{b5}{a6}
...
\cite[\textsection 3]{DBLP:journals/pacmpl/PulteFDFSS18}.
 In \ref{MCA2}, $(\DW{x}{1})$ is delivered to the first second thread, but not
...
is imposed.  Item \ref{5} also validates roach-motel reorderings \cite{SevcikThesis}.  For
...
Similarly, redundant write after read elimination is invalid \citet[\textsection
5.3-4]{SevcikThesis}:
\begin{displaymath}
