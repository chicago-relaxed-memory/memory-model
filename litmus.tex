...
box, including value range analysis, internal reads, and SC access, all of
which can be complex in other models.  In this section, we walk through
several litmus tests, valid rewrites and invalid rewrites.  The examples show
that rules \ref{rf3}--\ref{rf4} and \ref{5a}--\ref{5d} are understandable as
\emph{general principles}, not merely as the intersection of a series of
relations.
...
be introduced between the reads. See example \ref{addr}
...
\paragraph{Internal Reads and Value Range Analysis}
...
imposed by \ref{4b}.  The pomset becomes inconsistent if we change
...
example, the rules for relaxed writes are:
  \begin{align*}
    ...
    &&\textif x\neq y&&\hbox{}
  \end{align*}
  Relaxed reads are similar.  Redundant load elimination \eqref{RL} follows
