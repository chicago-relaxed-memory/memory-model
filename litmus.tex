...
through several valid rewrites, invalid rewrites, and litmus tests.
...
% \cite[\textsection2.3]{DBLP:conf/java/Pugh99}.  If $\aLoc\neq\bLoc$, then:
  ...
  analysis.  By composing \ref{RR} and \ref{RL}, we validate \ref{CSE}, from
...
\end{align*}
We discuss the inverse of \ref{CL} in \textsection\ref{sec:case}.
% \begin{align*}
%   \begin{gathered}
...
Some sequentially valid rewrites are invalid in a concurrent setting, such as
...
\todo{  Maybe not  Sevcik 5.3.4 Irrelevant Read Introduction.
...
model allows the following, which is disallowed by \citet[\textsection8.2]{Dolan:2018:BDR:3192366.3192421}:
\begin{gather*}
  \taglabel{SC1}
...
\paragraph{MCA} We present two examples that are hallmarks of \mca{}
architectures.  The analysis follows from a few simple principles.
% \begin{gather*}
