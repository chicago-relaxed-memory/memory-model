...
\begin{frame}[label=mm]
  ...
  \begin{gather*}
    ...
    \visible<3->{\SEMI y^\modeRA\GETS1 }
    \PAR
    x\GETS 3 \SEMI x\GETS 4\SEMI
    \visible<3->{r\GETS y^\modeRA\SEMI}
    ...
    \hbox{\begin{tikzinline}[node distance=1.5em]
        ...
        \visible<3->{\event{wy1}{\DW[\modeRA]{y}{1}}{right=of wx2}}
        \alt<2->{\nonevent}{\event}{wx3}{\DW{x}{3}}{below=of wx1}%below right=of wy0}
        \event{wx4}{\DW{x}{4}}{right=of wx3}
        \visible<3->{\event{ry}{\DR[\modeRA]{y}{1}}{right=of wx4}}
  ...
  \begin{columns}
    \begin{column}{0.4\textwidth}\small%
      \begin{itemize}\itemsep=0pt\parskip=0pt
        ...
        \item<3-> $\DW[\modeRA]{}{}$: release
        \item<3-> $\DR[\modeRA]{}{}$: acquire
        }
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}\small%
      \begin{itemize}\itemsep=0pt\parskip=0pt
        ...
        \item<2-> Sequential: 4
        \item<2-> Concurrent: 2\visible<2>{, 1}
        }
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}[label=oota]
  \frametitle{Out of \only<3->{\xout}{order} \only<3->{thin air (OOTA)}}
  What value can be written to $z$? 
  \begin{gather*}
    \alt<5->{      
      \visible<3->{\tag{OOTA2}\label{OOTA2}}
      \IF{x}\THEN y\GETS 1\FI
      \PAR
      r\GETS y \SEMI \IF{\alert{r}}\THEN x\GETS1 \SEMI z\GETS r\FI
    }{
      \visible<3->{\tag{OOTA1}\label{OOTA1}}
      y\GETS x
      \PAR
  ...
  \begin{columns}
    ...
    \begin{column}{0.5\textwidth}\small%
      \only<1->{\begin{itemize}\itemsep=0pt\parskip=0pt
        \item \only<3->{\sout}{May reorder $r\GETS y$ and $x\GETS1$}
          \begin{itemize}
          \item \only<3->{\sout}{no dependency}
          \item \only<3->{\sout}{in compiler}
          \item \only<3->{\sout}{on ARM}
          \end{itemize}
...
\begin{frame}[label=compiler]
  ...
  \begin{gather*}
    ...
    \visible<2->{\ELSE x\GETS\alt<7>{\alert{2}}{1} \FI}
    \only<1>{\tag{OOTA1}}
    \only<2-6>{\tag{$\star$}}
    \only<7>{\tag{OOTA3}}
  \end{gather*}
  ...
  \begin{gather*}
    ...
    \PAR
    r\GETS y\SEMI
    \IF{r}\THEN x\GETS \alt<7>{\alert{2}}{1}\SEMI z\GETS \alt<7>{\alert{2}}{1} \ELSE x\GETS\alt<7>{\alert{2}}{1} \FI
  \end{gather*}
  \onslide<4->
  lift common code
  \begin{gather*}
...
\begin{frame}[label=oota4a]
  ...
  \end{gather*}
  \sout{$x$ and $y$ can only be $0$ or $1$}
  \begin{gather*}
    \hbox{\sout{$
        y\GETS x
        \PAR
  ...
  \begin{gather*}
    \hbox{\sout{$
        ...
        $}}
  \end{gather*}
  \sout{commute independent statements}
  \begin{gather*}
    \hbox{\sout{$
        y\GETS x
        \PAR
...
\begin{frame}[label=oota4b]
  ...
  \begin{itemize}    
    \only<-3>{
    \item<2-> Existential justification: ARM\cmark, ($\star$)\cmark, (OOTA3)\cmark, (OOTA4)\xmark
      \begin{itemize}
      ...
      \end{itemize}
    \item<3-> Universal justification: ARM\xmark, ($\star$)\cmark, (OOTA3)\cmark, (OOTA4)\cmark
      \begin{itemize}
    ...
    \item In the JMM, OOTA5 \emph{``is type correct if it declares
      ...
      \begin{itemize}
      \item<5-> To prove safety, Lochbihler partitions memory by type
        \begin{itemize}
        \item<6-> \sout{Realistic memory allocation}
        \item<6-> \sout{Java Security Architecture (security-sensitive
            constants)}
        \end{itemize}
      \end{itemize}
    }
    \only<7->{
    \item A failure of compositional reasoning
      \begin{itemize}
        \item<8-> In OOTA4, every thread satisfies:
          \begin{itemize}
        ...
        \item<9->
          ...
          \end{math}
        \item<10-> Whole program should satisfy this
        \end{itemize}
      }
...
\begin{frame}[label=hard]
  ...
  \item<2-> Implementer desiderata
    \begin{itemize}
    \item Single-threaded compiler optimizations
    \item No hardware synchronization for relaxed access
      \visible<3->{\alert{on MCA architectures}}
    \end{itemize}
  \item<3-> Multi-copy atomicity (MCA)
    \begin{itemize}
    \item When write published to one processor, published to all
    \item ARMv8, x86-64, RISC-V, but not POWER
    \end{itemize}
    \vspace{-3mm}
    \onslide<4->
    \begin{gather*}
...
\begin{frame}
  ...
  \begin{gather*}
    ...
    \begin{aligned}
      ...
      \\ 
      \sem{\aReg\GETS\aLoc^{\visible<8->{\alert<8>{\amode}}}\SEMI \aCmd} & \eqdef \textstyle\bigcup_\aVal\;
      \alert<4>{(\DR[\visible<8->{\alert<8>{\amode}}]\aLoc\aVal)\prefix} \sem{\aCmd} \alert<3>{[\aLoc/\aReg]}
      \\
      \sem{\aLoc^{\visible<8->{\alert<8>{\amode}}}\GETS\aExp\SEMI \aCmd} & \eqdef
      \textstyle\bigcup_\aVal\; \alert<4>{(\aExp=\aVal \mid \DW[\visible<8->{\alert<8>{\amode}}]\aLoc\aVal)\prefix} \sem{\aCmd}\alert<3>{[\aExp/\aLoc]}
      \\
      \sem{\FENCE^{\visible<8->{\alert<8>{\fmode}}}\SEMI \aCmd} & \eqdef
      (\DFS{\visible<8->{\alert<8>{\fmode}}}) \prefix \sem{\aCmd}
      \\
  ...
  \visible<8->{
    \begin{align*}
      \alert<8>{\amode} \BNFDEF& \modeRLX &&\text{{(Relaxed)}}   &\alert<8>{\fmode} \BNFDEF&\modeREL  &&\text{(Release)}
  ...
  \begin{gather*}
    ...
    \begin{gathered}
      ...
      \hbox{\begin{tikzinline}[node distance=1.2em]
          ...
          \wk{a1}{a2}
          % \event{a3}{\DFS{\modeREL}}{right=of a2}
          % \sync{a2}{a3}
...
\begin{frame}[label=x]
  ...
  \item Elimination: Redundant fence, Redundant read,  Irrelevant read$^\star$
  \end{itemize}
\end{frame}
...
\begin{frame}[label=y]
  ...
  \item Store forwarding$^\star$, Dead store elimination$^\star$
  \item Dead code elimination
  \item Code lifting, Case analysis$^\star$
...
\begin{frame}[label=z]
  ...
  \item Valid$\cmark$ Proof$\xmark$
    ...
    \end{itemize}
  \item Observationally sound$\cmark$ Valid$\xmark$
    \begin{itemize}      
    ...
    \end{itemize}
  \item Observationally sound$\qmark$ Valid$\xmark$
    \begin{itemize}      
    \item Access mode weakening, eg $(\modeSC\rightarrow \modeRA\rightarrow\modeRLX)$
    \item Lock elision
    \end{itemize}
  \item Observationally sound$\xmark$
    \begin{itemize}
...
\begin{frame}
  ...
  \item Other results
    \begin{itemize}
    \item Compiles efficiently to ARM
  ...
  \item Limitations
    ...
    \item Mixed size access
    \item Limited optimizations
    \item Limited logic for OOTA
    \item MCA: ARM$\cmark$ Power$\xmark$
    \item No separate address dependencies
    \end{itemize}
  \end{itemize}    
\end{frame}
...
\begin{frame}
  \frametitle{Address Dependencies}
  \begin{gather*}
  \tag{ADDR2}
  \begin{gathered}
...
\begin{gather*}
  ...
  \event{f}{\DR[\modeRA]{z_2}{1}}{above=1.8em of z}
  \event{g}{\DR{x}{1}}{right=of f}
  \sync{a}{b}
  \po{c}{d}
  \sync{d}{e}
