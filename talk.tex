...
\begin{gather*}
  ...
  % x\GETS0\SEMI
  % z\GETS0\SEMI
  r\GETS y \SEMI \IF{\alt<1>{r}{b}}\THEN  x\GETS r \SEMI z\GETS r \ELSE x\GETS1 \FI
  \uncover<2->{\PAR
  %b\GETS0\SEMI
  b\GETS1}
  \\
...
\begin{frame}<2->
  ...
  \item 
    ...
    \item[{\labeltextsc[P4c]{(P4c)}{4c}}]
      if $\labelingAct(\bEv)=(\DW{..)}{}$ then $\labelingForm'(\aEv)$ implies $\labelingForm(\aEv)$
    \item[{\labeltextsc[P5a]{(P5a)}{5a}\labeltextsc[P5]{}{5}}]%
      if $\labelingAct(\aEv)=(\DW{..)}{}$ then $\labelingForm'(\aEv)$ implies $\labelingForm(\aEv)$ or $\bEv\le'\aEv$
    \item<1>[{\labeltextsc[P5b]{(P5b)}{5b}}]
      if $\bEv$ and $\aEv$ are conflicting actions then $\bEv\le'\aEv$
    \item<1>[{\labeltextsc[P5c]{(P5c)}{5c}}]
      if $\bEv$ is an acquire or $\aEv$ is a release then $\bEv \le' \aEv$
...
\begin{frame}
  ...
  Let $\aPS'\in(\aForm \mid \aAct) \prefix \aPSS$ when
    ...
    \item[{\labeltextsc[P5a]{(P5a)}{5a}\labeltextsc[P5]{}{5}}]%
      if $\labelingAct(\aEv)\mkern1.8mu=(\DW{..)}{\phantom{\aLoc}}$ then $\labelingForm'(\aEv)$ implies $\labelingForm(\aEv)$ or $\bEv\le'\aEv$
