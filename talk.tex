...
\begin{frame}
  ...
  \item 
    ...
    \item $\labeling: \Event \fun (\Formulae\times\Act)$ is a \emph{labeling}
      ...
      \begin{itemize}
      \item $\labelingForm:\Event\fun\Formulae$ %  --- formulae include $r{=}v$  $x{=}v$
      \item $\labelingAct:\Event\fun\Act$  %--- actions include $\DW{x}{v}$, $\DR{x}{v}$, and $\DSTOP$
      \end{itemize}
...
\begin{frame}<1-2>
  ...
  \item 
    ...
    \item[{\labeltextsc[P4b]{(P4b)}{4b}}]
      if $\bEv$ \externally reads $\aVal$ from $\aLoc$ then $\labelingForm'(\aEv)$ implies $\labelingForm(\aEv)[\aVal/\aLoc]$
    \item[{\labeltextsc[P4c]{(P4c)}{4c}}]
      if $\bEv$ does not read then $\labelingForm'(\aEv)$ implies $\labelingForm(\aEv)$
