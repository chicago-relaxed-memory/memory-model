...
\begin{frame}[label=oota4b]
  ...
  \vspace{-1\baselineskip}
  \begin{align*}
    \uncover<1-9>{y\GETS x
      \PAR&
      r\GETS y\SEMI
      \IF{r}\THEN
      x\GETS r\SEMI z\GETS r
      \ELSE x\GETS 1 \FI
      \only<1->{\spacetag{$\star$}\label{star}}}
    \\
    \uncover<1-9>{y\GETS x
      \PAR&
      r\GETS y\SEMI
  ...
  \begin{itemize}    
    ...
    \item In the JMM, OOTA5 \emph{``is type correct if it declares
      ...
      \visible<6->{
        ...
        \end{itemize}}
      \end{itemize}
    \visible<7->{\item OOTA is a \emph{queasy feeling}} \visible<8->{--- Compositionality is a \emph{verifiable property}}
    \visible<9->{
      \begin{itemize}
      \item Predicate logic rules out OOTA1-3
      \item Temporal logic rules out OOTA4
      \end{itemize}}      
    }
    \only<10->{
    \item In OOTA4, every thread satisfies:
      ...
      \end{itemize}
      \visible<11->{
      \item By compositionality, whole program should satisfy this
        \begin{itemize}
