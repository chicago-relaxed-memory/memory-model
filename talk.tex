    ...
    \item In the JMM, OOTA5 \emph{``is type correct if it declares
      ...
      \begin{itemize}
        ...
        \visible<6->{
          ...
          \end{itemize}}
      \end{itemize}
      \visible<7->{\item \emph{OOTA} is a \emph{queasy feeling}}
...
\begin{frame}
  ...
  \visible<1->{
    \begin{align*}
      \alert<1>{\amode} \BNFDEF& \modeRLX &&\text{{(Relaxed)}} &\alert<1>{\fmode} \BNFDEF&\modeREL  &&\text{(Release)} &\hbox{$\;\mkern60mu\;$}&
...
\begin{frame}[label=nonmca]
  ...
  \item 
    ...
    \end{description}
  \item 
    When $\bEv$ \emph{fulfills $\aEv$} (on $\aLoc$):
    \begin{description}
...
\begin{frame}[label=rmw]
  ...
  \begin{gather*}
    \begin{gathered}
      x\GETS 0 \bigl(\FADD^{\modeRLX,\modeRLX}(x,1) \PAR \FADD^{\modeRLX,\modeRLX}(x,1)\bigr)
      \\
    ...
    \\[1ex]
    \begin{gathered}
      \aLoc\GETS0\SEMI\bReg\GETS \FADD^{\modeRLX,\modeRLX}(\aLoc,1)
      \PAR
    ...
    \begin{gathered}
      ...
      r\GETS z\SEMI
      x\GETS 0\SEMI
      s\GETS \FADD^{\modeRLX,\modeRA}(x,1) \SEMI
      y\GETS s{+}1
      \\[-1ex]
      \hbox{\begin{tikzinline}[node distance=1.5em]
          \event{a1}{\DR{y}{1}}{}
