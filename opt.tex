...
single threaded optimizations including reordering of independent statements; and by showing that our model validates {\em all} valid transformation of a sequential program without synchronization into a ``linear'' sequential program.   A key technical tool in the latter proof is the connection to traditional Hoare logic for sequential programs, that formalizes the sense in which the thread-local order of our model captures no more than those forced by data dependencies.  
...
This invariant captured in the following theorem underlies the definitions in the semantics of sequential commands. In this theorem, we consider Hoare triples over formulas as per as per \textsection\ref{sec:data}.  We follow the treatment of Hoare logic as laid out in~\citet{gordonHoare}.  

In the hypothesis of this theorem, we consider valid state assignments $\vec{\bLoc} =\vec{\aVal}$, so there arent multiple conflicting assignments to the same location.  
...
The following Hoare triples is valid for all $i=1, \ldots m$. 
\begin{displaymath}
  \hoare{\aForm_i\land\bLoc^1_i=\aVal^1_i\land\cdots\land\bLoc^n_i=\aVal^n}{\aCmd}{\aLoc_i=\bVal_i}  
\end{displaymath}
if and only if
$\readc\sem{\aCmd}$ contains a pomset  that includes for all $i=1, \ldots, n$
\begin{tikzdisplay}[node distance=1em]
...
With any sequential and synchronization free  program fragment, we associate a ``linear'' program fragment $\linV{\aCmd}$ as follows.
\begin{definition}
Let $\aCmd$ be a sequential and synchronization free  program fragment.  
...
\begin{proof}
Proof follows immediately from the soundness and completeness of Hoare logic for sequential operational semantics;  see~\citet{gordonHoare} for the formalized proof for the version of the logic that we are using.
\end{proof}
\end{theorem}

 
\begin{theorem}\label{seqcompleteness}
Let $\aCmd$, $\bCmd$ be sequential programs such that $\bCmd$ is linear and
