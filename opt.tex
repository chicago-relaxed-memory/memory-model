...
\begin{definition}
  ...
  Let $\Event_\bLocs$ be the set of $\bEv_\bLoc$s.
  Let $\bSub$ be the substitution generated % from $\bLocs$
  as follows:
  % For $\bLoc\in\bLocs$, 
  $\bLoc\bSub = \aVal$ exactly when $\aEv_\bLoc$ writes $\aVal$ to $\bLoc$.

  Require that $\bForm\bSub$ be satisfiable.

  Require that if $\cEv\le\bEv_\bLoc$ and $\cEv$ is a read, then $\cEv\in\Event_\aLocs$.

  Require that if $\bEv_\bLoc\le\cEv$ and $\cEv$ is a write, then $\cEv\in\Event_\bLocs$.
\end{definition}
...
\begin{theorem}
  ...
  $\notdisjoint{\readsem{\aCmd}}{\semRW{\aForm}{\bForm}}$.
\end{theorem}
