...
\begin{math}
  x\GETS1 \SEMI z_1^\modeRA\GETS 1
  \PAR x\GETS2 \SEMI z_2^\modeRA\GETS 1
  \PAR \IF{z_1^\modeRA \land z_2^\modeRA} \THEN \hole{}\FI.
\end{math}
...
\begin{gather*}
  ...
  \end{gathered}
\end{gather*}
If we changed the read of $x^\modeRA$ to $x^\modeRLX$, then there would be no order from
$(\DR[\modeRLX]{x}{1})$ to $(\DR{\REF{1}}{0})$, and the execution would be allowed.  In
