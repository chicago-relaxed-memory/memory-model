...
We show that our semantics compiles efficiently to the ARMv8 model of
\citet{DBLP:journals/pacmpl/PulteFDFSS18}.  Efficient compilation is not
possible for earlier models of ARMv8.  For example, the model of
\citet{DBLP:conf/popl/FlurGPSSMDS16} allowed the following outcome
\cite[Fig.~4]{DBLP:conf/fm/LahavV16}, which is disallowed here:
\begin{gather*}
  %\taglabel{ARMv8-2016}
  \begin{gathered}
  ...
  \hbox{\begin{tikzinline}[node distance=1em]
      ...
      \rf{b}{c}
      \rf{d}{e}
      \rf[out=170,in=10]{f}{a}
    \end{tikzinline}}
\end{gathered}
\end{gather*}
This type of ``big detour'' \cite{alglave} is outlawed by
\citet{DBLP:journals/pacmpl/PulteFDFSS18}\footnote{There is either a cycle
...
In our language, past races are always resolved at a stable point.  Consider
...
The notion of ``future'' is also richer here.  Consider \cite[Example
