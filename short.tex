...
\begin{proof}[Proof Sketch]
    ...
    If there is $({\rpox}\cup{\rrfe})$-maximal access, we select it as
    $\aEv$.  If it is a read, we switch its fulfilling write to a ``newer'' one.
    For example, we switch from $\aPS''$ on the left to below to $\aPS'''$ on
    the right.  % Program order
    ...
    ``older'' one. As an example, consider the following:
    \begin{gather*}
