...
\item \emph{substitution} $\aPSS\aSub$, which applies the substitution to
  every precondition,
% \item \emph{restriction} $\nu\aLoc\DOT\aPSS$, which internalizes $\aLoc$ for
...
% Let $(\nu\aLoc\DOT\aPSS)$ be the set $\aPSS'$ where $\aPS'\in\aPSS'$ whenever
...
%   % Let $(\nu\aLoc\DOT\aPSS)$ be the set $\aPSS'$ where $\aPS'\in\aPSS'$
