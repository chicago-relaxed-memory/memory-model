...
\begin{example}
  ...
  $z$}.  Because our language lacks object creation, we cannot consider
  ...
  $z$}.  We show an attempted execution that violates
  this invariant:
...
\begin{gather*}
  ...
  x\GETS0\SEMI
  a\GETS0\SEMI
  \IF{z}\THEN  r\GETS y \SEMI x\GETS r \SEMI a\GETS r \ELSE x\GETS1 \FI
  \PAR
  z\GETS0\SEMI
    z\GETS1
  \\[-1ex]
\hbox{\begin{tikzinline}[node distance=1.25em]
  ...
  \rf[out=-170,in=-10]{wx}{rx}
  \po[out=25,in=25]{ry}{wa}
  \wk[out=25,in=155]{wy0}{wy}
  \wk[out=19,in=161]{wx0}{wx}
  \wk[out=19,in=161]{wa0}{wa}
  \rf[out=-170,in=-17]{wz1}{rz}
  \po[out=-160,in=-160]{rz}{wa}
\end{tikzinline}}
% \hbox{\begin{tikzinline}[node distance=1.5em]
%   \event{rx}{\DR{x}{1}}{}
...
% \end{tikzinline}}
  \end{gathered}  
  \end{gather*}
  We include location $a$ to indicate the branch taken.
  The outcome is disallowed, due to the cycle.  This outcome is also
  disallowed by our event structures model
  ...
  \emph{allowed} by \citep{DBLP:conf/esop/JagadeesanPR10},
  \citep{DBLP:conf/popl/KangHLVD17}, and
