...
\textsection\ref{sec:limits}, we discuss \oota{} examples that require
...
\begin{example}
  ...
  \ref{OOTA1} is: \emph{allocation at type $\classC$ is preceded by reading $0$ for
  $b$}.  Because our language lacks object creation, we cannot consider
  \ref{OOTA1} directly.  Instead we study \ref{OOTA4}, which has the same
  temporal structure: \emph{writing $1$ for $x$ is preceded by reading $0$ for
  $b$}.  We show an attempted execution that violates
  this invariant,  eliding initialization:
...
\end{tikzinline}}
  \end{gathered}  
\end{gather*}
Like \eqref{unexpected}, and \emph{unlike} \eqref{OOTA?}, there is a
dependency from $(\DR{y}{1})$ to $(\DW{x}{1})$.
  %We include location $z$ to indicate the branch taken.
Thus, the outcome is disallowed.  This outcome is also disallowed by our
event structures model \citep[\textsection9]{DBLP:journals/lmcs/JeffreyR19},
although the logic given in that paper is insufficient to establish this
fact.  The outcome is \emph{allowed} by
\citep{DBLP:conf/esop/JagadeesanPR10}, \citep{DBLP:conf/popl/KangHLVD17}, and
\citep{DBLP:journals/pacmpl/ChakrabortyV19}.
