...
Other \oota\ examples require that we extend the logic to deal with
intervening writes and
coherence. \citet[Fig.~3]{DBLP:journals/pacmpl/ChakrabortyV19} introduce
\weakestmo{} to disallow the following outcome, which is allowed by
the promising semantics %\textsc{promising}
\cite{DBLP:conf/popl/KangHLVD17}.
The outcome is disallowed in our semantics due to the evident cycle.
\begin{gather*}
  \label{weakestmo}
  x\GETS 2\SEMI
