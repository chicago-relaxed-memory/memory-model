...
\begin{align*}
  ...
  this structure: ``any write to location $a$ is preceded by a read of $1$
  for $z$.''  As a warmup, note that attempting to write $1$ to $a$ results
  in a cycle:
...
%   % presented a \labeltext{The}{page:logic:limits} logic we presented in
%   % \textsection\ref{sec:logic} is only strong enough to prove a few
%   % examples. different a logic, capable of showing that the following program
%   % cannot write 2: 
...
%   [\DW{y}{1} \Rightarrow \once\DR{x}{0}] \land
%   [\DW{x}{1} \Rightarrow \once\DR{y}{1}] \land
