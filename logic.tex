...
We develop a logic sufficient to prove that our semantics
disallows thin air executions, such as ``type unsafety'' (\textsection\ref{sec:intro}).
...
This simple principle is sufficient for ``type unsafety'':
...
\begin{align*}
  &\VAR x\SEMI x\GETS0\SEMI \VAR y\SEMI y\GETS0\SEMI \VAR z\SEMI z\GETS0\SEMI 
  \\[-1ex]
  &\quad(
    y\GETS x
  \PAR
...
We can also show that the ``random number
generator''~\cite{DBLP:conf/lics/JeffreyR16,DBLP:conf/esop/SvendsenPDLV18}
never writes $2$.
\begin{displaymath}
...
\end{displaymath}
The following invariant holds for each of the two threads, and thus, by
composition, for the aggregate program:
\begin{scope}
  \footnotesize
\begin{gather*}
...
Using the first conjunct, we have, as  required:
\begin{displaymath}
\DW{z}{2} \Rightarrow \FALSE
\end{displaymath}
