...
\citet{DBLP:journals/dc/Lamport86} considers a transitive, acyclic relation $\xpo$, and a relation $\xwk$ with the following axioms to connect $\xpo,\xwk$, namely:
\begin{align*}
\aEv \xpo \bEv \Leftarrow\;& \aEv \xwk \bEv\\
\aEv \xwk \bEv \Leftarrow\;& (\aEv \xwk \cEv \wedge\ \cEv \xpo \bEv) \vee\ (\aEv \xpo \cEv \wedge\ \cEv \xwk \bEv) \\
\aEv \xpo \bEv \Leftarrow\;& \aEv \xpo \bEv_1 \xwk \bEv_2 \xpo \bEv 
\end{align*}
In our setting, the basic ingredient of $\xwk$ is $\reco$, from which we can define the full $\xwk$ by closing by the above axioms. The $\xwk$ so defined is transitive.   Furthermore, if $\xpo$ is acyclic and follows the idioms arising from programs, then the induced $\xwk$ is also acyclic.   This leads us to the three-valued  memory pomsets that we have considered in this paper. 

In a concurrency context, three valued transition relations are studied in modal transition systems~\citep{DBLP:conf/lics/LarsenT88,DBLP:conf/avmfss/Larsen89} and the semantics of  three-valued temporal logics for such transition systems is studied in~\citep{DBLP:conf/concur/BrunsG00,DBLP:conf/esop/HuthJS01}. 
