...
\emph{synchronization-free} if lacks fences and $\mRA/\mSC$ access.

We consider an extension of Hoare logic with read sets.  The format of triples we will consider is:
\[ \hoare{\aLin\SEMI\aForm}{\aCmd}{\bLin\SEMI\bForm} \]
$\aLin, \bLin$ are multisets of the shared variable bindings of the form $\aLoc = \aVal$.  

The triple is intended to capture the the following intended partial correctness semantics.  We only consider terminating computations of $\aCmd$ 
\begin{itemize}
\item Initial state satisfies $\aLin\ \land\ \aForm $ 
\item $\bLin\ \subseteq\ \aLin$
\item Final state satisfies $\bForm$
\item If the computation reads a value, say $\aVal$, for $\aLoc$ that it did not write itself, say $n$ times,  then $\aLin \setminus \bLin$ contains the multiset with at least $n$ occurrences of 
$\aLoc = \aVal$  
\end{itemize}

The rules are as follows.
\begin{itemize}
\item Structure. Weakening
\[
\frac{\aForm_1 \Vdash\ \aForm, \bForm_1 \Vdash\ \bForm,\aLin_1 \supseteq\ \aLin, \bLin \subseteq\ \bLin_1, \hoare{\aLin_1\SEMI\aForm_1}{\aCmd}{\bLin_1 \SEMI\bForm_1}}{ \hoare{\aLin\SEMI\aForm}{\aCmd}{\bLin\SEMI\bForm}}
\]

\item  Structure.  Connectives.
\[ \frac
{\hoare{\aLin\SEMI\aForm_1}{\aCmd}{\bLin\SEMI\bForm_1},  \hoare{\aLin\SEMI\aForm_2}{\aCmd}{\bLin\SEMI\bForm_2}}{ \hoare{\aLin\SEMI \aForm_1 \lor \aForm_2}{\aCmd}{\bLin\SEMI\bForm_1\lor \bForm_2}}
\]

\[ \frac
{\hoare{\aLin\SEMI\aForm_1}{\aCmd}{\bLin\SEMI\bForm_1},  \hoare{\aLin\SEMI\aForm_2}{\aCmd}{\bLin\SEMI\bForm_2}}{ \hoare{\aLin\SEMI \aForm_1 \land \aForm_2}{\aCmd}{\bLin\SEMI\bForm_1\land \bForm_2}}
\]

\item Conditional.
\[\frac
{
    \hoare{\aLin\SEMI\aForm \land\ \aExp}{\aCmd}{\bLin\SEMI\bForm}
    \quad
    \hoare{\aLin\SEMI\aForm\ \land\ \lnot\aExp}{\bCmd}{\bLin\SEMI\bForm}
  }{
    \hoare{\aLin\SEMI\aForm}{\IF{\aExp}\THEN \aCmd \ELSE \bCmd\FI}{\bLin\SEMI\bForm}
  }
\]

\item Sequential composition
\[ \frac
{\hoare{\aLin\SEMI\aForm}{\aCmd}{\aLin_1\SEMI\aForm_1}, \hoare{\aLin_1\SEMI\aForm_1}{\bCmd}{\bLin\SEMI\bForm}}
{\hoare{\aLin\SEMI\aForm}{\aCmd \SEMI \bCmd}{\bLin\SEMI\bForm}}
\]

\item Assignment.  External read.
\[
\hoare{\aLin \cup \{\aLoc = \aVal \}\SEMI\aForm[\aVal/\aReg]}{\aReg \GETS \aLoc}{\aLin \SEMI\aForm}
\]

\item Assignment.  Internal read.  External write.  Register.
\[
\hoare{\aLin\SEMI\aForm[\aLoc/\aReg]}{\aReg \GETS \aLoc}{\aLin \SEMI\aForm}
\]
\[
\hoare{\aLin\SEMI\aForm[\aReg/\aLoc]}{\aLoc \GETS \aReg}{\aLin \SEMI\aForm}
\]
\[
\hoare{\aLin\SEMI\aForm[\aExp/\aReg]}{\aReg\GETS\aExp}{\aLin\SEMI\aForm}
\]
...
We develop a pomset semantics for pairs of formulae
$\semRW{\aLin\SEMI\aForm}{\bLin\SEMI\bForm}$, and show a relation between $\sem{\aCmd}$ and
$\semRW{\aLin\SEMI\aForm}{\bLin\SEMI\bForm}$ for valid  triples
$\hoare{\aLin\SEMI\aForm}{\aCmd}{\bLin\SEMI\bForm}$. 

In this section, we will only consider {\em globally consistent} pomsets, ie. those such that 
\[ (\bigwedge_{\aEv}\labelingForm(\aEv)) \wedge\ \bigwedge_{\aEv} \aLoc = \aVal \mid \labelingAct(\aEv) = \DR{\aLoc}{\aVal} \]
is satisfiable.  

% In \textsection\ref{sec:model}, preconditions were discharged by read events.  Here, instead, preconditions
% are derived from the read events themselves.

\begin{definition}
  \label{def:prepost}
  Let $\aLin\land\aForm$ be satisfiable, $\aLin\ \supseteq \bLin$, and $\bForm$ is a conjunction of a multiset of atoms of the form $\bLoc= \aVal'$.

$\aPS\in\semRW{\aLin\SEMI\aForm}{\bLin\SEMI\bForm}$
...
\begin{itemize}
\item  Let $\bLocs$ be the set of locations referred to in $\bForm$.  For each $\bLoc\in\bLocs$, choose
  $\aEv_\bLoc\in\Event$ that writes the $\bLoc$.
  Let $\aEvs_\bLocs=\{\aEv_\bLoc\mid\bLoc\in\bLocs\}$.
 
\item For each $\aLoc\GETS\aVal\in\aLin$, choose a distinct
  $\bEv_\aLoc\in\Event$ that reads $\aLoc$ with value $\aVal$.
  Let $\bEvs_\aLocs=\{\bEv_\aLoc\mid\aLoc\in\aLocs\}$.
 
\item Require that if $\cEv\le\aEv_\bLoc$ and $\cEv$ is a read, then $\cEv\in\aLin\setminus\bLin$.
\item Require that if $\aEv_\bLoc\le\cEv$ and $\cEv$ is a write to $\bLocs$, then $\cEv\in\aEvs_\bLocs$.
\end{itemize}
\end{definition}
Pictorially, we have:
...
\end{align*}
% \eqref{WW}, \eqref{RW} and \eqref{RR} require that two sides of the semicolon
% have disjoint ids; for example, \eqref{RW} requires $\disjoint{{\free(\aReg
%     \GETS \aLoc)}}{{\free(\bLoc \GETS \bExp)}}$. 
\textcoor{red}{\eqref{RR} requires either $\aReg\neq\bReg$ or
...
\textcoor{red}{Since reads are unordered in our model, read optimizations are not limited by
