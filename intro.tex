...
The JMM and the related models of~\citet{DBLP:conf/esop/JagadeesanPR10}, \citet{DBLP:conf/popl/KangHLVD17} and~\citet{Chakraborty:2019} invalidate compositional reasoning, as demonstrated by variants of \citeauthor{DBLP:journals/toplas/Lochbihler13}'s type unsafety example.  Thus these approaches fail to support either type safety or realistic memory allocation.
...
As far as we are aware, ours is the first language-level model to capture \mca.   The appeal of \mca{} in hardware is the dramatically simpler programmer model.  We believe it has the same appeal at the language-level.  
