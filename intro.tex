...
As a result of such \emph{bait-and-switch} behaviors, the JMM does not allow
compositional reasoning for temporal safety properties.  Each individual
thread of \ref{OOTA1} satisfies the invariant
% \emph{allocation at type $\classD$ is preceded by reading zero for $z$}.
% They also satisfy:
\emph{allocation at type $\classC$ is preceded by reading a nonzero value for
  $z$}.  Yet the parallel composition of these threads does not.  This lack
of compositionality forced \citeauthor{DBLP:journals/toplas/Lochbihler13} to
partition memory by type in order to prove type safety.  This formal device
means that memory cannot be used at different types over time, making
practical memory reclamation impossible.  Even with this solution for type
safety, there are implications for the Java security architecture
