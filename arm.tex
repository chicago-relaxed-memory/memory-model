...
acylicity of $\lt$ is clear.  By the definition of $\gtN$, it is also clear
...
definition~\ref{def:3valued} and the constraints on top-level pomsets arising
from definition~\ref{def:x-closed}.  Both of these constraints are coherence
...
\begin{remark}[Proof for TSO]
  ...
  three key facts listed above hold for \tso, where $\rob$ is replaced by (the
