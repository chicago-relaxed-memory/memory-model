...
This motivates the translation of an \armeight\ execution into our setting.  In our setting, the progress of time is given by $\lt$, and we accommodate intra-thread reordering by internal read actions, thus excusing us from the obligation of placing them on the global timeline associated with $\lt$.  
...
permissive than the syntactic dependency used in \armeight.  So, the heart of the proof is showing the acyclicity of $({\rob} \cup {\reco})^*$ for the events under consideration.  Recall that the cross thread portion of $\reco$ ($\rcoe,\rfre,\rrfe$) is included in $\rob$.  So, the acyclicity result is really a statement about the intra-thread portion of $\reco$.  The key feature facilitating this is our translation of internal reads, which renders them internal, thus removing them from consideration.    Thus, in the proof of acyclicity in \textsection\ref{sec:arm:proof}, we only have to consider ${\rob}$ that was derived without ever using  $\rrfi$; in this \emph{constrained} fragment of \armeight, we show the following key fact.
