...
permissive than the syntactic dependency used in \armeight.  So, the heart of the proof is showing the acyclicity of $({\rob} \cup {\reco})^*$ for the events under consideration.  The key feature facilitating this is our translation of internal reads.  Thus, in the proof of acyclicity in \textsection\ref{sec:arm:proof}, we only have to consider ${\rob}$ that was derived without ever using  $\rrfi$; in this \emph{constrained} fragment of \armeight, we show the following key fact.
