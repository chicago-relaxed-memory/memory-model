...
\item ${\sle} \subseteq {\gtN}$ is a partial order. 
  \end{itemize}
\end{definition}
We write $\bEv\slt\aEv$ when $\bEv\sle\aEv$ and $\bEv\neq\aEv$, and similarly for $\gtN$.   Thus, $(\sle \cup \reco)^{\star} \subseteq {\gtN}$.  
...
\item Let $\reco$ be the restriction of $\gtN$ to conflicting actions on the same location.  Then, $(\Event, {\sle}, (\sle \cup \reco)^{\star}, \labeling)$ is a \tvalpom, and $(\sle \cup \reco)^{\star} \subseteq {\gtN}$.
\end{itemize}
