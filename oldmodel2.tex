...
$\aLoc$.
% The \emph{mode} $\amode$ is either \emph{relaxed} ($\mRLX$),
% \emph{release-acquire} ($\mRA$) or \emph{sequentially-consistent} ($\mSC$).
...
We elide the $\mRLX$-mode annotation in examples.
% We write $\mREL$ and $\mACQ$ as synonyms for $\mRA$.
\begin{comment}
...
% \\[-1ex]\BNFSEP& \REF{\cExp}^\mRA\GETS\aExp\SEMI \aCmd \tag{Releasing write to memory}
% \\[-1ex]\BNFSEP& \aReg\GETS\REF{\cExp}\SEMI \aCmd \tag{Relaxed read from memory}
% \\[-1ex]\BNFSEP& \aReg\GETS\REF{\cExp}^\mRA\SEMI \aCmd
...
% We write $^\mRA\aLoc\GETS\aExp\SEMI \aCmd$ as shorthand for $\FENCE_{\FR} \SEMI\aLoc\GETS\aExp\SEMI \aCmd$.
% We write $^\mRA\aReg\GETS\aLoc\SEMI \aCmd$ as shorthand for $\aReg\GETS\aLoc\SEMI \FENCE_{\FA}\SEMI \aCmd$.
...
For complete programs, we can restrict attention to events that are either
...
At top-level, fulfillment imposes a total order on conflicting writes that
...
\begin{gather*}
  \begin{aligned}
    \;\;&\labelingAct'(\aEv) = \labelingAct^1(\aEv) = \labelingAct^2(\aEv) \textAnd \labelingForm'(\aEv) \textimplies \labelingForm^1(\aEv) \lor \labelingForm^2(\aEv),\\[-1ex]
    &\;\;\aEv \not\in \Event^2,\; \labelingAct'(\aEv) = \labelingAct^1(\aEv) \textAnd \labelingForm'(\aEv) \textimplies \labelingForm^1(\aEv),\; \textor\\[-1ex]
    &\;\;\aEv \not\in \Event^1,\; \labelingAct'(\aEv) = \labelingAct^2(\aEv) \textAnd \labelingForm'(\aEv) \textimplies \labelingForm^2(\aEv), and
  \end{aligned}
  \\
\begin{aligned}
...
These operators validate the equation:
\begin{displaymath}
  \sem{\IF{\aExp} \THEN \aCmd \ELSE \bCmd \FI}
  =
  \sem{\IF{\aExp} \THEN \aCmd \FI \PAR \IF{\lnot\aExp} \THEN \bCmd \FI}
...
\!$\PAR$\! in the definition of write is necessary to validate \emph{case
  analysis}, discussed in \textsection\ref{sec:opt}.
% \begin{math}
%   \sem{\aCmd}
%   =
...
\begin{align}
  ...
  \end{gathered}
\end{align}
By drawing convention, these are:
...
We now relax item 5 of Candidate \ref{def:pre-sc} so that only some program
...
\item[6.] if $\bEv$ is an acquire then $\labelingForm(\aEv)$
...
\begin{gather*}
    ...
    x\GETS 1\SEMI y^\mRA\GETS1 \PAR r\GETS y^\mRA; s\GETS x
    \\[-1ex]
    \hbox{\begin{tikzinline}[node distance=1em]
        \event{wx0}{\DW{x}{0}}{}
        \event{wx1}{\DW{x}{1}}{right=of wx0}
...
%This example is also disallowed if we use $\mSC$ rather than $\mRA$.
...
$\mSC$.  It is also immediate that SC actions can be totally ordered.
...
relaxed access in this section, extending to $\mRA$/$\mSC$-access after
...
\begin{definition}
  ...
  \begin{align*}
    ...
    \;\mathhl{\cup\;
      % (\iDRmode{\aLoc}{}) \prefix
      \text{if $\amode\mathbin\neq\mRLX$ then }\text{$\emptyset$ else }
      \sem{\aCmd}[\aLoc/\aReg]
      % \text{ else $\emptyset$}
    }
  \end{align*}
...
\begin{gather*}
  x\GETS1 \SEMI
  a^\mRA\GETS1 \SEMI
  \IF{b^\mRA}\THEN  y\GETS x \FI
  \PAR
  \IF{a^\mRA}\THEN  x\GETS 2\SEMI b^\mRA\GETS1 \FI
  \\
