...
The definitions of substitution, restriction and guarding and write filtering
...
Let $(\DW{\aLoc}{} \guard \aPSS)$ be the subset of $\aPSS$ such that $\aPS\in\aPSS$ whenever
there is some $\aEv$ that writes $\aLoc$ such that for every release
$\bEv\in\Event$, $\aEv\leq\bEv$.
\end{definition}
...
We only consider programs where generally, register state is empty in forked
