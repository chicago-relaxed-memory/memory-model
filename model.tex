...
% \begin{definition}
  ...
  \end{itemize}
\end{definition}
The axioms for $\gtN$ are from~\citet{DBLP:journals/dc/Lamport86}.
The last two requirements are new.
...
each node $\aEv$ is labeled with $\labeling(\aEv)$.
We visualize $(\bEv \lt \aEv)$ as a \emph{strong edge},  $\bEv \xpo \aEv$.
We visualize$(\bEv \geN \aEv)$ as a \emph{weak edge},  $\bEv \xwk \aEv$.
% For example:
...
\item control dependencies coalescing states
\end{itemize}
