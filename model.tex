...
\item formulae are closed under negation, conjunction, disjunction, and
  substitution\footnote{Since formulae are closed under substitutions of the
    ...
    $(\aEExp=\aVal)$  where $\aEExp$ is an \emph{extended expression} that includes memory locations.}, and
   %and $(\REF{\aEExp}=\aLoc)$,
   % We elide the details.  By composition of the closure
    % conditions, formulae must also be closed under that substitutions of the
    % form $\aForm[\aExp/\aReg]=\aForm[\aLoc/\aReg][\aExp/\aLoc]$.
...
are closed with respect to \emph{augmentation}, \emph{implication}, and
\emph{prefixes}: %, and \emph{isomorphism}:
\begin{definition}
  Let
  ...
  $\Event' \supseteq \{ \bEv \in \Event \mid \exists\aEv\in\Event'.\; \bEv\le\aEv\}$,
...
\begin{definition}
  ...
  Let $\aPS' \in (\aPSS^1 \parallel \aPSS^2)$
  when there are $\aPS^i \in \PRE(\aPSS^i)$ %and $\aPS^2 \in \PRE(\aPSS^2)$
  such that
$\Event' = \Event^1 \cup \Event^2$,
...
\begin{candidate}
  ...
  $\aPS'\in\aPSS'$ when $\aPS'=\emptypomset$ or there is $\aPS\in\aPSS$ such that $\aPS'$ adds a
...
$\aPS'\in\aPSS'$ when $\aPS'=\emptypomset$ or
there is $\aPS\in\aPSS$ such that:
\begin{enumerate}
...
Let $(\aForm \mid \aAct) \prefix \aPSS$ be the set $\aPSS'$ where
$\aPS'\in\aPSS'$ when
$\aPS'=\emptypomset$ or
there is $\aPS\in\aPSS$ such
that satisfies items 1-4 of
...
intervening writes are respected:
\begin{math}
  \sem{x\GETS 0\SEMI x\GETS 1\SEMI y\GETS x}
\end{math}
does not $\DW{y}{0}$.
