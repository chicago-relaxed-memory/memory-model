...
by $\aSub$ and $\bSub$.  We write substitutions
$[\bExp_1/\aLR_1,\ldots, \bExp_n/\aLR_n]$.  The empty substitution is written
as $[\,]$.
...
\begin{definition}
  ...
  \item ${\le} \subseteq (\Event\times\Event)$ is a partial order, and
    ...
    % \end{enumerate}
  \item ${\ltN} \subseteq (\Event\times\Event)$ such that:
    \begin{itemize}
    \item\label{5a} if $\bEv \le \aEv$ then $\aEv \ltN \bEv$,
    \item\label{5b} if $\bEv \le \aEv$ and $\bEv \ltN \aEv$ then $\bEv = \aEv$,
...
We write pairs in $(\Formulae\times(\Act\cup\Sub))$ as $(\aForm \mid \aAct)$ or
$(\aForm \mid \aSub)$.
We elide $\aForm$ when $\aForm$ is a tautology, and write $\aAct$ crossed-out ($\NEVER\aAct$)
...
\end{array}\]
We visualize $(\aEv \ltN \bEv)$ as a dashed
...
\begin{eqnarray*}
  \sem{\SKIP} & = & [\,]\\
  \sem{\aLoc\GETS\aExp} & = & \textstyle\bigcup_\aVal\; \bigl((\aExp=\aVal) \guard (\DW\aLoc\aVal) \prefix [\aExp/\aLoc]\bigr) \\
  \sem{\aReg\GETS\aLoc} & = & [\aLoc/\aReg] \cup \textstyle\bigcup_\aVal\; (\DR\aLoc\aVal) \prefix [\aLoc/\aReg] \\
  \sem{\IF (\aExp) \THEN \aCmd \ELSE \bCmd \FI} & = & \bigl((\aExp \neq 0) \guard \sem{\aCmd}\bigr) \parallel \bigl((\aExp=0) \guard \sem{\bCmd}\bigr) \\
  \sem{\aCmd \SEQ \bCmd} & = & \sem{\aCmd} \sequence \sem{\bCmd} \\
  \sem{\aCmd \PAR \bCmd} & = & \sem{\aCmd} \parallel (\sem{\bCmd}\fork) \\
...
\subsection{Forking and Substitution}

Forking and substitution each perform a simple transformation on each pomset
in a set of pomsets.
...
\subsection{Guarding and Restriction}
\label{sec:restriction}

Guarding and restriction filter the set of pomsets; we have
$(\nu\aLoc\st\aPSS)\subseteq\aPSS$ and $(\aForm\guard\aPSS)\subseteq\aPSS$.

The definition of guarding is straightforward:
\begin{definition}
Let $(\aForm \guard \aPSS)$ be the subset of $\aPSS$ such that $\aPS\in\aPSS$ whenever:
\begin{itemize}
\item if $\labelling(\aEv) = (\bForm \mid \aAct)$ then $\aForm$ implies $\bForm$, and
\item if $\labelling(\aEv) = (\bForm \mid \aSub)$ then $\aForm$ implies $\bForm$.
\end{itemize}
\end{definition}

The definition of restriction is more complex.
...
\item if $\labelling_0(\aEv) = (\aForm_0 \mid \aAct)$ then either:
  ...
  \end{itemize}
\item if $\labelling_0(\aEv) = (\aForm_0 \mid \aSub)$ then either:
  \begin{itemize}
  \item $\labelling_1(\aEv) = (\aForm_1 \mid \aSub)$ and $\labelling_2(\aEv) = (\aForm_2 \mid \aSub)$
    and $\aForm_0$ implies $\aForm_1 \lor \aForm_2$,
  \item $\labelling_1(\aEv) = (\aForm_1 \mid \aSub)$ and $\aEv \not\in \Event_2$
    and $\aForm_0$ implies $\aForm_1$, or
  \item $\labelling_2(\aEv) = (\aForm_2 \mid \aSub)$ and $\aEv \not\in \Event_1$
    and $\aForm_0$ implies $\aForm_2$.
  \end{itemize}
\end{itemize}
\end{definition}
