...
\begin{definition}
  \label{def:3valued}
  A \emph{3-valued pomset with preconditions} %over $(\Formulae\times\Act)$
  is tuple $(\Event, {\le}, {\gtN}, \labeling)$, such that
  \begin{itemize}
  ...
  \item ${\gtN} \subseteq (\Event\times\Event)$ such that:
    ...
    \item\label{5b} if $\bEv \le \aEv$ and $\aEv \gtN \bEv$ then $\bEv = \aEv$, and  \hfill (Consistency)
...
% \begin{definition}
  ...
  \label{def:3pre}
  A \emph{(memory model) pomset} is a 3-valued pomset with preconditions, such that
  \begin{itemize}
  \item $\gtN$ is acyclic, and \hfill (Weak-acyclicity)
  \item if $\bEv\le\aEv$ then $\labelingForm(\aEv)$ implies
    $\labelingForm(\bEv)$. \hfill (Causal-strengthening)
  \end{itemize}
\end{definition}
The axioms for $\gtN$ are adapted
from~\citet[A1--A3]{DBLP:journals/dc/Lamport86}.  The last two requirements
are new.  We write $\bEv\lt\aEv$ when $\bEv\le\aEv$ and $\bEv\neq\aEv$, and
similarly for $\gtN$.  \citeauthor {DBLP:journals/dc/Lamport86} writes $\lt$
as $\xpo$ and pronounces it as ``precedes''; we also refer to $\lt$ as
``strong order''.  \citeauthor {DBLP:journals/dc/Lamport86} writes $\geN$ as
$\xwk$ and pronounces it as ``can affect''; given our additional acyclicity
requirement, we also refer to $\geN$ as ``weak order''.
...
$\labeling(\aEv)$, and the orders are drawn with \citeauthor
{DBLP:journals/dc/Lamport86}'s notation.
% For example:
...
\begin{definition}
  ...
  every $\aLoc$ it reads.
  % $\aLoc$ from $\bEv$,
  % for some $\bEv$, whenever $\aEv$ reads $\aLoc$.
\end{definition}
