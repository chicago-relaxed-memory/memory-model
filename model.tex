...
\[\begin{array}{c}
  ...
  0 \le 1 \quad
  0 \le 2 \quad
  \labeling(0) = \DR{\aLoc}{1}) \\
  \labeling(1) = \DW{\bLoc}{0}) \quad
  \labeling(2) = \DW{\bLoc}{1}) \quad
\end{array}\]
...
\item a set of \emph{expressions} $\Exp$, ranged over by
  $\aExp$ and $\bExp$,
\item a set of \emph{memory locations}, ranged over by $\aLoc$ and
  $\bLoc$.
...
\item memory locations are tagged values of the form $\REF{\aVal}$,
...
A top-level, we expect the use of each variable to be \emph{closed} and \emph{coherent}.
...
\label{def:x-closed}
  ...
  \end{itemize}
  A pomset is $\aLoc$-\emph{coherent} if, when restricted to events which
  touch $\aLoc$, $\gtN$ forms a partial order.
\end{definition}
...
isomorphism, augmentation and implication:
...
\end{definition}
NOTE: because implication closed, any event can go false, and we kill everything
...
\item \emph{restriction} $\nu\aLoc\st\aPSS$, which filters $\aPSS$ to
  include only pomsets where every event $\aEv$ that reads from $\aLoc$ can
  read from some $\bEv$ and where no precondition can depend on $\aLoc$,
...
The definitions of substitution, guarding and restriction are
...
\item if $\aAct$ is an acquire or $\labelingAct(\aEv)$ is a release then $\cEv \lt' \aEv$, and
