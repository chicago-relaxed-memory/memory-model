...
$\modeRA$ and $\modeSC$ reads are acquires; $\modeRA$ and
...
definition of prefixing more carefully this time.  We highlight the changes:
...
\vspace{-1ex}
    ...
    % \tag{$\dagger$}\label{sc-read}
    \sem{\aReg\GETS\aLoc^\amode\SEMI \aCmd} & =
    \smash{\textstyle\bigcup_\aVal\; (\DRmode\aLoc\aVal) \mathhl{\prefixsc}\; \sem{\aCmd}\mkern3.5mu \mathhl{[\aLoc/\aReg] }}
    \\
    % \tag{$\ddagger$}\label{sc-write}
    \sem{\aLoc^\amode\GETS\mathhl{\aExp}\SEMI \aCmd} & =
    \smash{\mathhl{\textstyle\PAR_\aVal\; (\aExp=\aVal} \mid \DWmode\aLoc\aVal) \mathhl{\prefixsc}\; \sem{\aCmd}}
  \end{align*}
\end{candidate}

% In the definition of prefixing,
Item 1 introduces a new event\nofootnote{Item 5 ensures that
  $\cEv\notin\Event$.  We relax item 5 in Definition \ref{def:prefix},
  allowing $\cEv\in\Event$.}.  Item 2 ensures that no order is removed from old
...
\begin{align}
  ...
  \end{gathered}
\end{align}
Using our conventions for drawing pomsets, these are:
...
Thus we do not require an explicit read action when a relaxed read is matched
by a local write.  Symmetrically, we do not require an explicit write action
when a relaxed write is only used to match local reads.  We first give the
final semantics of read.
\begin{definition}
  \label{def:rw:local}
  \begin{align*}
    \sem{\aReg\GETS\aLoc^\amode\SEMI \aCmd} & =
...
resulting in a tautology.  By removing the read event, this outcome is
...
\begin{gather*}
  ...
  \rf[out=10,in=170]{a1}{b2}
    \end{tikzinline}}
\end{gather*}
Symmetrically, item 6 in the definition of prefixing
(Candidate~\ref{def:prefix}) ensures that local reads are not matched to
