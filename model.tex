...
\begin{gather*}
  ...
  \wk[out=20,in=160]{wy0}{wy0a}
  \po{rx0}{wy0a}
  \sync{wx1}{stop}
  \sync[bend left]{ry0}{stop}
      \end{tikzinline}}
  \end{gather*}
...
This model is based on that of~\cite{2019-sp}, though missing the features designed to capture
...
We do not explicitly include C-style \emph{plain} access.  Plain access is
...
\begin{definition}
  We say that $\aForm$ is \emph{independent of $\aLoc$} when, for every
  ...
  We say that $\aForm$ is \emph{location independent} if it is independent of
  every location.
%
...
% \begin{itemize}    
% \item
\labeltext[F1]{(F1)}{rf1} $\bEv$ \externally writes $\aVal$ to $\aLoc$, 
% \item
\labeltext[F2]{(F2)}{rf2} $\aEv$ \externally reads $\aVal$ from $\aLoc$,
% \item
\labeltext[F3]{(F3)}{rf3} $\bEv \lt \aEv$, and
% \item
\labeltext[F4]{(F4)}{rf4} for every conflicting write $\cEv$, either $\cEv
...
element of the carrier set $\Event$ to left of its label.  Consider:
\begin{align*}
\begin{gathered}
  \IF{r<0}\THEN y\GETS1\FI
  \\
  \hbox{\begin{tikzinline}[node distance=1em]
  \eventl{e}{wx1}{r<0\mid\DW{y}{1}}{}
    \end{tikzinline}}
\end{gathered}
&&
\begin{gathered}
  ...
  \\
  \hbox{\begin{tikzinline}[node distance=1em]
  \eventl{e}{wx2}{r\geq0\mid\DW{y}{1}}{right=of wx1}
    \end{tikzinline}}
\end{gathered}
\end{align*}
...
\paragraph{Conditional, Register Assignment, and Skip}
...
$\stopPS$ is the set of pomsets that each contain a single termination event.
\begin{definition}
...
\todo{explain how the semantics handles this:}
\begin{verbatim}
{true} x=1; r=x; y=r {y=1}
\end{verbatim}
...
% \begin{candidate} %\ \vspace{-1ex}
  ...
  \begin{gather*}
    \begin{aligned}[t]
      ...
      \;\smash{\mathhl{\textstyle\bigcup_\aVal\; (\aExp=\aVal}\, \mid
        \DWmode\aLoc\aVal) \prefixsc\; \sem{\aCmd}}
    \end{aligned}
  \end{gather*}
\end{candidate}
The semantics of read and write introduce a pomset for each possible value.
Note that the value in write actions is fixed.  The dependence on $\aExp$
appears in the precondition: $\aExp=\aVal$.

Item \ref{1} introduces a new event.  Item \ref{2} ensures that no order is
removed from old events. Item \ref{3} specifies the action labelling the new
and old events, and item \ref{4} specifies the preconditions.  Item \ref{5}
ensures that program order is included for the new event.
