...
\item formulae are closed under negation, conjunction, disjunction, and
  substitutions of the form $\aForm[\aLoc/\aReg]$ or $\aForm[\bExp/\aLoc]$, 
...
$\aLoc$,  $(\DWRel{\aLoc}{\aVal})$, which is a release that writes $\aVal$
to $\aLoc$, and $(\DF)$, which is both an acquire and a release and neither
reads nor writes.
...
We fix the alphabet $\Alphabet=(\Formulae\times\Act)$.  
...
We write pairs in $(\Formulae\times\Act)$ as $(\aForm \mid \aAct)$.
We elide $\aForm$ when $\aForm$ is a tautology.
...
\begin{definition}
For a substitution $\aSub$ of the form $[\aLoc/\aReg]$ or
$[\bExp/\aLoc]$,  let $\aPSS\aSub$ be the set $\aPSS'$ where $\aPS'\in\aPSS'$ whenever
...
requirement $\bEv<\aEv$ in Definition~\ref{def:rf} by $\bEv\gtN\aEv$. It
remains to be seen how this impacts the model.
...
$\aPS$ is $\aLoc$-closed and $\aLoc$-coherent.
\end{definition}
