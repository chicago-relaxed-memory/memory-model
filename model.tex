...
then there would be order; this behavior would be disallowed due to the
evident cycle:
\begin{tikzdisplay}[node distance=1em]
  \event{a1}{\DR{x}{1}}{}
  \event{a2}{\DWRel{y}{1}}{right=of a1}
...
\item $\finternalize(\aAct)$ writes $\bot$ from $\aLoc$ exactly when $\aAct$ writes from $\aLoc$,
...
$(\iDRAcq{\aLoc}{\aVal})$, $(\iDR{\aLoc}{\aVal})$,
$(\iDWRel{\aLoc}{\aVal})$, and $(\iDW{\aLoc}{\aVal})$.
...
\item\label{pre-iread} if $\aAct$ internally reads from $\aLoc$ then both
  ...
  \item[(\ref{pre-iread}b)] if $\aEv$ is a write then either $\cEv\lt'\aEv$
...
\item\label{pre-coherence} if $\aAct$ and $\labelingAct(\aEv)$ are external actions in conflict,
    then $\cEv \gtN' \aEv$,
...
\begin{align*}
  ...
  \textstyle\bigcup_{\aLoc=\REF{\cVal}}\; \textstyle\bigcup_\aVal\; ({\cExp}=\cVal \mid \DRAcq\aLoc\aVal) \prefix \sem{\aCmd}[\aLoc/\aReg] 
  \\
  \sem{\aReg\GETS\REF{\cExp}\SEMI \aCmd} & =
  \textstyle\bigcup_{\aLoc=\REF{\cVal}}\; \textstyle\bigcup_\aVal\; ({\cExp}=\cVal \mid \DR\aLoc\aVal) \prefix \sem{\aCmd}[\aLoc/\aReg] 
  ...
  \textstyle\bigcup_{\aLoc=\REF{\cVal}}\; \textstyle\bigcup_\aVal\; ({\cExp}=\cVal \land \aExp=\aVal \mid \DWRel\aLoc\aVal) \prefix \sem{\aCmd}[\aExp/\aLoc] 
  \\
  \sem{\REF{\cExp}\GETS\aExp\SEMI \aCmd} & =
  \textstyle\bigcup_{\aLoc=\REF{\cVal}}\; \textstyle\bigcup_\aVal\; ({\cExp}=\cVal \land \aExp=\aVal \mid \DW\aLoc\aVal) \prefix \sem{\aCmd}[\aExp/\aLoc]
