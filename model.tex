...
\emph{fulfilled} concurrently by a matching write (Definition \ref{def:rf}),
preconditions represent an obligation that must be \emph{satisfied}
sequentially via substitution.
...
As we shall see, it is only possible to weaken this precondition by
introducing order from $(\DR{y}{1})$ to $(\DW{x}{1})$ (requirement \ref{5a}
of Definition \ref{def:prefix}).
...
%
  ...
  \item Actions in $\Acq$, $\Rel$ and $\SC$, are \emph{synchronization} and
    ...
    % $\fdom(\rreads)\cap\SC\subseteq\Acq$ and 
    % $\fdom(\rwrites)\cap\SC\subseteq\Rel$.
...
% (Definition\ref{def:mmpomset}), pomsets do not include contradictory
% preconditions:
...
(Def.~\ref{def:mmpomset}).  However, the definition allows events with
the same label to \emph{coalesce}, taking the disjunction of their
...
guaranteed by downset closure on the right pomset above.  Consistency
prevents any pomset in $\sem{\aCmd_{\mathsf{cond}}}$ from containing both
$(\DW{x}{2})$ and $(\DW{x}{3})$.
...
Prefixing adds a new read or write event to the beginning of a pomset.  As
for composition, we allow coalescing.  The candidate definition given here
fails to validate some compiler optimizations.  We give the final definition
...
introduces a new event, which may coalesce with an existing event in
$\Event$.  \ref{2} ensures that no order is removed from old events.  \ref{3}
specifies the actions labelling the events.  \ref{4} specifies the
preconditions (along with \ref{5a}).  \ref{5} specifies the preserved program
order.
...
% \end{gathered}
% \end{gather*}
%the difference with \eqref{unexpected} %and \eqref{OOTA!}
%can be seen in
%the semantics of
\begin{math}
...
forces a dependency from the read of $y$ to the write of $x$.
% In an execution that reads $b{=}1$, \ref{5a} again forces a dependency
% from the read of $y$ to the write of $x$.
...
% of $(r{=}1\mid\DW{x}{1})$ using \ref{4c} when prefixing $(\DR{y}{1})$. This
...
\begin{align*}
  \hbox{\begin{tikzinline}[node distance=1em]
      \event{wx}{(r{=}1\lor b{=}0)\land (r{=}1\lor 1{=}0)\mid\DW{x}{1}}{}
      \event{wz}{(r{=}1\lor b{=}0)\land (r{=}1\lor 1{=}0)\mid\DW{z}{1}}{right=of wx}
...
At the point that $\aLoc$ is bound, we require that every read of $\aLoc$
...
discuss coherence further in \textsection\ref{sec:litmus}.
