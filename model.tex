...
%
  ...
  \item We say that action $\aAct$ is a \emph{read} if
    ...
    $\aAct\in\fdom(\rwrites)$.  When $\rreads(\aAct) = (\aLoc,\aVal)$, we say
    that $\aAct$ \emph{reads} $\aVal$ \emph{from} $\aLoc$, and similarly for
    writes.  We say that $\aAct$ \emph{accesses} $\aLoc$ if it either reads
    or writes $\aLoc$.
...
We model \emph{executions} as \emph{pomsets with preconditions},
...
We model \emph{programs} (notation $\sem{\aCmd}$) as \emph{sets} of pomsets with
...
% \item
\labeltext[F4]{(F4)}{rf4} for every conflicting write $\cEv$, either $\cEv \gtN \bEv$ or $\aEv \gtN \cEv$.
% \end{itemize}
\end{definition}
Item \ref{rf3} requires that a write $\bEv$ is ordered before any read $\aEv$ it
fulfills; we draw order required by this item (aka \emph{reads-from}) with
bold green arrows.  Item \ref{rf4} requires that any conflicting write $\cEv$ is
...
the expression $\aExp$.  Note that the value of the write action is fixed.
The dependence on $\aExp$ is moved into the disjoint preconditions: $\aExp=\aVal$.
