...
%
  ...
  $\aAct\in\Acq$, a \emph{release} if $\aAct\in\Rel$, a \emph{termination} if
  $\aAct\in\Term$,  and \emph{SC} if
...
% $\SC\cap\fdom(\rwrites)\subseteq\Rel$,
% and
We also require that termination events do not read or write, and are not
acquiring, releasing, or SC.
%$\disjoint{\Term}{(\Acq\cup\Rel\cup\SC\cup\fdom(\rreads)\cup\fdom(\rwrites))}$.
...
postcondition must therefore imply the postcondition of all other events.
