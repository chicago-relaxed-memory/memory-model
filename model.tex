...
\begin{definition}
  ...
  \end{itemize}
\end{definition}
In the parlance of chapter 3.3 of~\citet{AlglaveThesis}, $\le$ is a ``global happens-before'' relation.  
%We write $\bEv\lt\aEv$ when $\bEv\le\aEv$ and $\bEv\neq\aEv$, and similarly for $\gtN$.
% \citet{DBLP:conf/esop/HuthJS01} call $\lt$ a ``must transition''
...
another.  This relation always goes \emph{from writes to reads across
  threads}.  We consider reads-from to be a strong order, which we visualize
with a thick arrow ``$\xrf$''.  Since new threads may introduce order from
