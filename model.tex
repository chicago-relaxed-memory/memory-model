...
\begin{definition}
  \label{def:dis}
  We say that $\aPS$ is an \emph{disjunct} of $\aPS^i$ ($i\in I$) if there is
...
precondition taken from the two sides.  Composition is used define
...
$\aPSS$ that imply $\aForm$.  Register assignment is defined using
...
Definition \ref{def:pre-sc}.  A pomset must preserve program order in four
cases: \ref{5a} weakening a dependent precondition, \ref{5b} conflict (aka,
coherence), \ref{5c} $\modeRA$-fences, and \ref{5c} $\modeSC$-fences.
...
Items \ref{5d} ensures that program order between SC operations is always
...
consequence of substitution.  The \armeight{} model has to a jump through
...
\labeltext{As}{page:inlining} side note, \ref{Internal2} shows that---like most relaxed models---our model
