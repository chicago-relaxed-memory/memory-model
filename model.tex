...
The assignment to $\aReg$ causes a substitution in the precondition, changing
the label to $(x=1\mid \DW{y}{1})$.  By introducing strong order
$(\DR{x}{1})\xpo(x=1\mid\DW{y}{1})$, this precondition can be weakened by
further substitution of $1$ for $x$, resulting in $1=1$, which is a tautology
and therefore elided.

As in \citep{2019-sp}, weakening of preconditions can also happen by merging
actions.  For example, the semantics of
$\IF{\aReg}\THEN y\GETS1 \ELSE y\GETS1\FI$ includes a pomset with the single
action $(\DW{y}{1})$.  In computing the dependencies for the conditional
statement, we first compute the then-part to contain
$(\aReg\neq0\mid\DW{y}{1})$ and the else-part to contain
$(\aReg=0\mid\DW{y}{1})$.  Since the actions are the same in these events,
...
Between threads, $\geN$ defines a partial order on conflicting actions.
...
in cycles in weak order.  (In the following example, we include fences to
...
\begin{displaymath}
  x\GETS0\SEMI z\GETS0\SEMI x\GETS 1\SEMI z \REL\GETS1 \PAR r\GETS z\ACQ; s\GETS x
\end{displaymath}
...
\begin{tikzdisplay}[node distance=1em]
  ...
  \event{wx1}{\DW{x}{1}}{right=of wx0}
  \event{wz1}{\DWRel{z}{1}}{right=of wz0}
  \event{rz1}{\DRAcq{z}{1}}{right=2.5em of wz1}
  \event{rx1}{\DR{x}{1}}{above=of rz1}
  \po{wx0}{wz1}
  \po{wz0}{wz1}
  \po{wx1}{wz1}
  \po{rz1}{rx1}
...
with a plain write, then the outcome $(\DRAcq z1)$ and $(\DR x0)$ is possible:
\begin{tikzdisplay}[node distance=1em]
  \event{wx0}{\DW{x}{0}}{}
  \event{wz0}{\DW{z}{0}}{below=of wx0}
  \event{wx1}{\DW{x}{1}}{right=of wx0}
  ...
  \wk{rx0}{wx1}
\end{tikzdisplay}
since no order is required between $(\DW x1)$ and $(\DW z1)$.  
Symmetrically, if we replace the acquire of the original program
with a plain read, then the outcome $(\DR z1)$ and $(\DR x0)$ is possible.
...
action need be recorded.  To facilitate the definitions required in the proof
...
location.  To see the need for internal reads, consider the example 3.6 from
...
\begin{tikzdisplay}[node distance=1em]
  ...
  \rf{b2}{a1}
\end{tikzdisplay}
Were the internal action made explicit, then this behavior would be
disallowed due to the evident cycle in strong order.  This behavior is
allowed by \armeight.

Here is an example which uses a cycle in weak order rather than strong order.
We also use a fence (not drawn) rather than an acquire or release:
\begin{displaymath}
  y \GETS0 \SEMI \FENCE\SEMI x\GETS1\SEMI a\GETS y
  \PAR
  x \GETS0 \SEMI \FENCE\SEMI y\GETS1\SEMI b\GETS x
\end{displaymath}
\begin{tikzdisplay}[node distance=1em]
  \event{a1}{\DW{y}{0}}{}
...
\paragraph{Multi-copy atomicity.}  The standard litmus tests for \mca\ are
...
this execution is allowed.  If there is a fence between the reads, however,
this execution is disallowed do to the evident cycle in weak order.
...
\end{math}
has top level executions that result in the final state $x = y = 1$, such as:
\begin{tikzdisplay}[node distance=1em]
  % \event{wx0}{\DW{x}{0}}{}
  ...
  %\po{rx}{wy}
\end{tikzdisplay}
However, the execution becomes impossible if there is strong order from read
to write in both threads, as in
\begin{math}
  (y\GETS x \PAR x\GETS y).
\end{math}
In \textsection\ref{sec:logic} we provide machinery to prove this.
...
thread arises out of thin air.   In our model, the attempted execution is:
\begin{tikzdisplay}[node distance=1em]
  \event{rx}{\DR{x}{1}}{}
  \event{wy}{\DW{y}{1}}{below=of rx}
  \po{rx}{wy}
