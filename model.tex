...
\begin{tikzpicture}[baselinecenter,node distance=1em]
  ...
  \internal{rx1}{\DR{x}{1}}{left=of wy1}
\end{tikzpicture} 
...
In the pomset on the left, the read is \emph{internal}, and therefore most be
discharged by a preceding action in the same thread.  In the pomset on the
right, the read is \emph{external}.  An external read may discharge the
precondition on a event that is ordered after: by introducing order
$(\DR{x}{1})\xpo(x=1\mid\DW{y}{1})$, we may substitute $1$ for $x$ in the
precondition, resulting in $1=1$, which is a tautology and therefore elided.
