...
% \item if $\aAct$ reads then either $\labellingForm'(\aEv)$ implies $\labellingForm(\aEv)$ or $\cEv<'\aEv$,  %
...
prefixing.
\begin{itemize}
\item $\fpo(\aPSS\aSub)=\fpo(\aForm\guard\aPSS)=\fpo(\aPSS)$
\item $\fpo(\aPSS_1\parallel\aPSS_2)=\fpo(\aPSS_1)\cup\fpo(\aPSS_2)$
\item $\fpo(\nu\aLoc\st\aPSS)=\fpo(\aPSS)$ but restricted to the events in
  the restriction
\item $\fpo(\aAct\prefix\aPSS)$ just adds the new event to $\fpo(\aPSS)$
\end{itemize}

$\fird$ is similar: you get $\fird$ from the prefixed event $\cEv$ to an old event
$\aEv$ if $\labellingForm(\aEv)$ is dependent on $\aLoc$ (ie, it is
