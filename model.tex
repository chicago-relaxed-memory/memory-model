...
the semantics of a concurrent language.  The semantics given here is slightly
simplified.  As discussed in \textsection\ref{sec:props}, it fails to
...
\end{tikzinline}}
\end{gather*}
An attempt to replicate the unexpected execution of~\eqref{OOTA?} fails,
...
% The precondition on $\DW{x}{1}$ is required by the data dependency induced by
% $\bReg$ (see page \pageref{page:strengthening}).
Note that, by causal strengthening, the termination event of a completed
pomset must imply all other events.
...
% \item[(3b)] $\labelingForm'(\bEv)$ implies $\aForm$,
% \item[(4a)] $\labelingAct'(\aEv) = \labelingAct(\aEv)$,
\item[{\labeltext[4a]{(4a)}{4a}}]{\labeltext[4]{}{4}} $\labelingForm'(\bEv)$ implies $\aForm$, 
\item[{\labeltext[4b]{(4b)}{4b}}] if $\bEv$ \externally reads $\aVal$ from $\aLoc$ then
  $\labelingForm'(\aEv)$ implies $\labelingForm(\aEv)[\aVal/\aLoc]$,
\item[{\labeltext[4c]{(4c)}{4c}}] if $\bEv$ does not \externally read then
  $\labelingForm'(\aEv)$ implies $\labelingForm(\aEv)$, 
\item[{\labeltext[5a]{(5a)}{5a}}] %$\bEv$ \externally reads and
  \labeltext[5]{if}{5} $\aEv$ writes then either $\bEv\lt'\aEv$ or $\labelingForm'(\aEv)$ implies $\labelingForm(\aEv)$,
...
The unexpected execution of \ref{OOTA?}:
\begin{gather*}
  y\GETS 0 \SEMI x\GETS 0\SEMI (y\GETS x
  \PAR
  r\GETS y\SEMI \IF{r}\THEN 
...
In the next section, we explain item \ref{5} using standard litmus tests:
Read fulfillment is constrained by the program order that is preserved by
\ref{5}.
...
In the remainder of this section, we concentrate on the sequential semantics:
Preconditions are satisfied using the substitutions in the semantic rules and
in \ref{4b}.  We close by discussing the interaction between preconditions
and \ref{5a}.
