...
unsatisfiable---true or false.  Substitution into preconditions arises via
prefixing.  Reading $x$, for example, we arrive at:
\begin{align*}
  \begin{gathered}
...
%\paragraph{Fulfillment and Variable Declaration.}
At the point a variable is declared, every read must be \emph{fulfilled} by a
matching write.
...
\item\label{pre-order} $\cEv \lt' \aEv$, 
\end{enumerate}
\end{definition}
...
there is $\aPS\in\aPSS$ that satisfies the first \ref{dis-int1} items of
