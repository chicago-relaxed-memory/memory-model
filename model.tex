...
In isolation, the write action is $(\bReg=1\mid\DW{x}{1})$.  As in the
discussion of execution \eqref{m1}, the strong order from $(\DWRel{y}{1})$
allows the precondition to be weakened to $1=1$, which is a tautology.

\textcolor{red}{TODO: show how dependency calculated}
...
\item\label{pre-write} if $\aAct$ is neither reads nor writes, then $\labelingForm'(\aEv)$
  implies $\labelingForm(\aEv)$,
\item\label{pre-read} if $\aAct$ reads or writes $\aVal$ from $\aLoc$ then
  \begin{enumerate}
