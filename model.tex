...
\citet{2019-sp} defined a model of computer \emph{microarchitecture}, in
order to capture Spectre \cite{DBLP:journals/corr/abs-1801-01203} and similar
security flaws that arise from speculative evaluation.  We build on their
work to define a model of relaxed memory \emph{architecture}.  Interestingly,
the model naturally captures multi-copy atomicity (\mca).
...
Actions that acquire or release are \emph{synchronizations}, other actions
are \emph{relaxed}.
...
$\aForm$ when it is a tautology.  We write $\bEv\lt\aEv$ when $\bEv\le\aEv$
and $\bEv\neq\aEv$, and similarly for $\gtN$.

Within a single thread, $\lt$ captures the notion of \emph{dependency}:
$\bEv\lt\aEv$ indicates that these events cannot appear in the reverse order.
In the absence of synchronization, this relation always goes from reads to
writes.  The formula decorating the event is used to calculate these
single-threaded dependencies using semantic entailment.

Between threads, $\lt$ indicates that one thread has \emph{read-from}
another.  This relation always goes from writes to reads.  Since new threads
may introduce order from write to read, we define our semantics to be closed
with respect to augmentation of $\le$.

We give the semantics of a program as a set of pomsets.  At top level, we
expect that every read event is fulfilled by a matching write.  We define
fulfillment in terms of $\gtN$, rather than $\le$, so that it is monotone
with respect to addition of new threads.  This suffices since $\bEv\geN\aEv$
implies that we cannot have $\aEv\lt\aEv$ in any augmentation. (In
\textsection\ref{app:blockers} we show that the obvious definition of
fulfillment in terms of $\le$ does not enjoy this property.)
...
\begin{definition}
  ...
  \end{itemize}
  We say that $\aEv$ is \emph{fulfilled} if $\aEv$ \emph{reads $\aLoc$ from}
  some $\bEv$ whenever $\aEv$ reads $\aLoc$.
\end{definition}

Within a single thread, $\geN$ orders conflicting events: Two actions
