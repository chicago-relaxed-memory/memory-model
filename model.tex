...
the partial order tracks causality, which arises from various types of dependency, coherence, and synchronization.
...
\begin{itemize}
\item $(\DR{y}{1})\xpo(\DW{x}{1})$ is a \emph{dependency} requirement,
...
The same calculation fails for the write to $x$ in \eqref{OOTA!}.  In this case,
the writes to $x$ on either side of the conditional cannot be combined, since
once side writes $1$ and the other side writes $2$.  Thus, the semantics of
the conditional contains $(r{=}1 \mid \DW{x}{1})$, rather than
$(r{=}0 \lor r{=}1 \mid \DW{x}{1})$.  It is only possible to weaken this
...
In each pomset, the value appearing in the action is fixed.  For writes, the dependence on
$\aExp$ appears in the precondition: $\aExp=\aVal$.

Writes introduce preconditions that must be satisfied using substitution and
in \ref{4b}.  Reads introduce a fulfillment requirement, which is constrained
by the program order that is preserved by \ref{5}. Requirement \ref{5a}
connects the sequential semantics of write to the concurrent semantics of read.
%\ref{5a} captures \emph{read to write  dependency}.
It only requires order from read to write\footnote{When $\bEv$ is not a
  read, \ref{4c} trivially implies \ref{5a}.} when the precondition of the
write is \emph{weakened} using \ref{4b}.  The other requirements in \ref{5}
are standard: \ref{5b} captures the extended coherence requirement on actions
that touch the same location.  \ref{5c} imposes the order required by acquire
and release actions.\footnote{Recall that termination actions are releases.}
\ref{5d} imposes the additional order required by SC actions.\footnote{Recall
  that SC reads are acquires and SC writes are releases.}

In the remainder of this section, we focus on the sequential semantics.  We
explain the concurrent semantics in the next section using standard litmus tests.
