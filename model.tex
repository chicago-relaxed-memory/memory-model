...
\begin{tikzdisplay}[node distance=1em]
  ...
  \po{rx1}{wy1}
\end{tikzdisplay}
We refer to edges introduced by $(\bEv \leq \aEv)$ as \emph{strong edges} and
by $(\bEv \gtN \aEv)$ as \emph{weak edges}.
Reads-from is included in strong order, and we often highlight it in red, as above.
...
\begin{align*}
  ...
  \textstyle\bigcup_{\aLoc=\REF{\bVal}}\; ({\cExp}=\bVal) \guard \textstyle\bigcup_\aVal\; (\DRAcq\aLoc\aVal) \prefix \sem{\aCmd}[\aLoc/\aReg] 
  \\
  ...
  \\ & \mkern2mu\cup \textstyle\bigcup_{\aLoc=\REF{\bVal}}\; ({\cExp}=\bVal) \guard \mkern21mu\; (\iDR{\aLoc}{0}) \prefix \sem{\aCmd}[\aLoc/\aReg]
  \\
  \sem{\REFRel{\cExp}\GETS\aExp\SEMI \aCmd} & =
  \textstyle\bigcup_{\aLoc=\REF{\bVal}}\; ({\cExp}=\bVal) \guard \textstyle\bigcup_\aVal\;  (\aExp=\aVal) \guard\bigl((\DWRel\aLoc\aVal) \prefix \sem{\aCmd}\bigr)[\aExp/\aLoc] 
  \\
  \sem{\REF{\cExp}\GETS\aExp\SEMI \aCmd} & =
  \textstyle\bigcup_{\aLoc=\REF{\bVal}}\; ({\cExp}=\bVal) \guard \textstyle\bigcup_\aVal\;  (\aExp=\aVal) \guard\bigl((\DW\aLoc\aVal) \prefix \sem{\aCmd}\bigr)[\aExp/\aLoc]
  %\\ & \mkern2mu\cup \textstyle\bigcup_{\aLoc=\REF{\bVal}}\; ({\cExp}=\bVal) \guard \DW{\aLoc}{} \guard \sem{\aCmd}[\aExp/\aLoc]
  \\
