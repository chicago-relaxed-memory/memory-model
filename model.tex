...
\begin{definition}
  ...
  \end{itemize}
\end{definition}
In the parlance of chapter 3.3 of~\citet{AlglaveThesis}, $\le$ is a ``global happens-before'' relation.  The restriction of $\lt$ to conflicting writes is called $\rco$, the coherence order.  
...
A write event $\aEv$ has two kinds of dependencies.  First, the external dependency on reads $\bEv$ that is reflected as $\bEv  \lt \aEv$.  Second, it has internal dependencies reflected in the formula decorating $\aEv$.  In order for the write event to fire, both these dependencies need to be satisfied.  The external dependency on a read is satisfied by globally visible writes.   The internal dependency can only be fulfilled by a preceding  write in the same thread.  
...
\begin{math}
  ...
  y\GETS \aReg
\end{math}.
In this special case when the value is read from a write of the same thread, as in the above example, as in \citep{2019-sp}, the semantics includes:
\begin{tikzdisplay}[node distance=1em]
  \event{wx1}{\DW{x}{1}}{}
  \event{wy1}{\DW{y}{1}}{right=of rx1}
\end{tikzdisplay}
...
As in \citep{2019-sp}, weakening of preconditions\footnote{We say a formula $\psi$ is weaker than a formula $\phi$ if $\phi \Rightarrow \psi$.}can also happen by merging
...
this execution is acyclic and is allowed.  
...
\end{displaymatharray}
where the first read in each thread is acquiring.  In this case, there is strong order between the reads; the resulting
execution is shown below and disallowed due to the evident cycle.
\begin{tikzdisplay}[node distance=1em]
  \event{wx0}{\DW{x}{0}}{}
  \event{wx1}{\DW{x}{1}}{right=of wx0}
  \event{wy0}{\DW{y}{0}}{below=4ex of wx0}
...
\begin{tikzdisplay}[node distance=1em]
  ...
  \wk{wz0}{wz1}
  \event{ry1}{\DR{y}{1}}{left=of wy}
  \rf{wy}{ry1}
  \event{wa}{\DR{a}{1}}{left=of ry1}
  \po{ry1}{wa}
\end{tikzdisplay}
...
inlining. \textsection\ref{sec:examples} develops some of them.
...
\end{definition}
We do not expect $\sem{\aCmd}$ to be prefixed closed; thus, one may view each
$\aPS\in\sem{\aCmd}$ as a \emph{completed} execution.  However, implication
...
Definition~\ref{def:mmpomset}.  Since unsatisfiable events are ignored by our
``architectural'' model (as opposed to a ``microarchitectural'' model that also models speculations that are rolled back), this
...
The semantics is also closed with respect to various forms of ``input
enabling,'' as described in \textsection\ref{drfproof}.
...
$\bEv\l
e\aEv$, for every release $\aEv\in\Event$.
\end{definition}
...
thread-local reads do not cross acquire actions.  This prevents bad executions
like the following:
\begin{displaymath}
  x\GETS1 \SEMI
  a\REL\GETS1 \SEMI
...
semantics, the values of internal actions are ignored.  In fact, internal
actions themselves are entirely ignored.  We include internal actions on read
only to define data races in \textsection\ref{sec:sc}.
...
The rule for relaxed writes is similar. Internal write is only allowed if
there is a subsequent write that masks the internal write.  For this reason
the internal write can only be applied to pomsets that satisfy the filter
$\DW{\aLoc}{}\guard\sem{\aCmd}[\aExp/\aLoc]$.  This filter requires that
there is some write to $\aLoc$ that precedes every release.
