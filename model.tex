...
simplest\footnote{Anticipating Definition \ref{def:prefix}, we include
  preconditions and substitutions.} candidate definitions of read and write
...
We explain the substitutions in the subsections below.
...
For a negative example, consider the following candidate execution for the
...
definition of fulfillment.  This candidate execution is not a pomset 
...
example.  From \eqref{sc-write}, $\sem{y\GETS r}$ contains
...
\end{enumerate}
\end{definition}
Item 1 introduces the new event\footnote{Anticipating Definition
  \ref{def:prefix}, the new event may overlap with an old one.}.  Item 5
(which we refine next) ensures that program order is included in pomset
order.  Item 2 ensures that no order is removed.  Item 3 describes the label
of the new event $\cEv$.  Item 4 describes the label of old events $\aEv$.

We can now explain the substitution in the semantics of read
\eqref{sc-read}.
...
In order to allow relaxed executions, such as the non-SC store-buffering
example given previously, we relax item 5 of Definition \ref{def:pre-sc} to
include only \emph{preserved} program order:
\begin{definition}
  \label{def:prefix}
...
Program order is not imposed between reads or between writes to different
locations.  From read to write, program order is only imposed in the case
that a precondition of the write is discharged.  Thus we have:
\begin{displaymath}
...
\end{displaymath}
\begin{tikzdisplay}[node distance=1em, baseline={([yshift=-1.5ex]current bounding box.west)}]
  \event{wx0}{\DW{x}{0}}{}
  %\event{wy0}{\DW{y}{0}}{below=of wx0}
  \event{wx1}{\DW{x}{1}}{right=of wx0}
...
Similarly, items 5c and 5d ensure that every pomset can be extended to totally
order SC accesses.


\paragraph{Internal reads and writes.}

\begin{definition}
  Let $(\relfilt{\modeRLX}{\aLoc} \aPSS)$ be the subset of $\aPSS$ such that
  $\aPS\in\aPSS$ whenever every release $\aEv$ has a preceding $\bEv\le\aEv$
  that \externally writes $\aLoc$.  For $\amode\neq\modeRLX$, let
  $(\relfilt {\amode}{\aLoc} \aPSS)$ be the empty set.
  \begin{align*}
    \sem{\aReg\GETS\aLoc^\amode\SEMI \aCmd} & =
...
We can now explain the substitution in the semantics of write
\eqref{sc-write}.  This substitution allows local writes to discharge
preconditions without introducing dependency.
For example, we have:
\begin{displaymath}  
  x\GETS1\SEMI r\GETS x\SEMI y\GETS r
\end{displaymath}
\begin{tikzdisplay}[node distance=1em]
  \event{wx}{\DW{x}{1}}{}
