...
Similarly, given $\aFn \in \pomFn$, we can generate $\aPS_{\aFn}$ as follows:
\begin{itemize}
\item $\Event$, the set of events,  is induced by the actions in $\aRSet_{\aFn} \cup \aFn(\aRSet_{\aFn})$.  Each of these events are labeled with the evident actions, namely the read (resp. write) events are associated with  associated with the read (resp. write) actions.    In the rest of this discussion, we conflate the event with its label.
...
The semantics of read is ``input''-enabled, ie. it permits the read of any visible value.   Thus, any racy read in a program can be replaced by a read of a earlier value (wrt the $\reco$), even while  the races with existing independent writes are maintained. 
...
\item $\aEv'$ reads from $\aLoc$, with matching write event $\bEv'$, such that $\bEv' \gtN \bEv$ in $\bPS$
\item The restriction of $\le$ (resp. $\gtN$) in $\aPS$ to $\Event_{\aPS} \setminus  (\aPS\ \after\ \aEv)$ agrees with the the restriction of $\le$ (resp. $\gtN$) in $\aPS$ to $\Event_{\bPS} \setminus  (\bPS\ \after\ \aEv)$  in  $\bPS$.  
\end{itemize}
\end{lemma}
In particular, $\bEv,\bEv'$ are events of both $\aPS$ and $\bPS$, and  $\bPS$ continues to witness the race of $\aEv', \bEv$ in $\bPS$.   
...
%\Event_{\bPS}$ is the disjoint union of  $\Event_{\aPS} \setminus  
%(\aPS\ \after\ \aEv))$ and $(\bPS\ \after\ \aEv')$.
\item  If $\aEv'$ reads from $\aLoc$ with matching write event $\bEv'$ in $\bPS$, and $\bEv' \gtN \bEv$ in $\bPS$ then $\bEv' \in 
\Event_{\aPS} \setminus \bigcup_i (\aPS\ \after\ \aEv_i)$. 
\item The restriction of $\le$ (resp. $\gtN$) in $\aPS$ to $\Event_{\aPS} \setminus \bigcup_i (\aPS\ \after\ \aEv_i)$ agrees with the the restriction of $\le$ (resp. $\gtN$) in $\aPS$ to $\Event_{\bPS} \setminus \bigcup_i (\aPS\ \after\ \aEv_i)$  in  $\bPS$.  
\end{itemize}
\end{lemma}
