\section{Other Related Work}\label{sec:ldrf}
...
disallow all two-location variants.  This odd behavior stems from
\emph{semi\hyp{}transitivity}, inherited from \citet{DBLP:journals/dc/Lamport86}.
...
First, in \textsection\ref{sec:opt}, we formalized a connection between Hoare logic
and the sequential fragment of our language, allowing us to make a general
statement about transformations.  Thus, we present a solution to a problem that has
been open since \citet{DBLP:conf/ecoop/SevcikA08} discovered that several sequential
compiler optimizations were invalid in the Java Memory Model
\cite{Manson:2005:JMM:1047659.1040336}.

Second, in \textsection\ref{sec:logic}, we proposed a cyclical proof rule for
...
prescription of \emph{essential dependencies}.  This provides a
...
Ours is the first language-level model to incorporate \emph{multi-copy
  atomicity} (\mca).  However, our main contributions do not depend on \mca.
It is possible to apply our ideas to stronger models, and to weaker ones.
For stronger models,
such as \citet{Boehm:2014:OGA:2618128.2618134} and \citet{Dolan:2018:BDR:3192366.3192421}, it is
likely sufficient to require more order in the pomset.  To compile
efficiently to weaker architectures, such as \ppc\ and \armseven, it 
appears necessary to use a model with more than one ordering relation.

The logic we presented in \textsection\ref{sec:logic} is only strong enough
...
$(\DR{y}{1})$.   This example requires the ability to reason about values.
...
% \end{math}
% as required.
As another example, the following outcome is allowed by the promising
semantics \cite{DBLP:conf/popl/KangHLVD17}, but not by \weakestmo{}
...
\begin{gather*}
  ...
  \wk{wx2}{rx3}
    \end{tikzinline}}
\end{gather*}
Is not possible for the left thread to read $3$ for $x$ when the right thread
reads $2$.  Proving this may require a logic with modalities to deal with
intervening writes and coherence.
