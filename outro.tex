...
because there is no following write.  At top-level, however, the read of $x$
must be fulfilled by \emph{some} write---assuming that these writes can
coalesce, the optimization can be validated.
...
access by generalizing $\relfilt[]{\aLoc}$ (Def.~\ref{def:cover}.)
