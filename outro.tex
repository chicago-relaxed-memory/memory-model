...
characterization optimizations, such as redundant read elimination, which
...
We have not attempted validate all transformations that involve
synchronizations and fences
...
(\ref{RL}), and store forwarding (\ref{SF}).  We expect that dead store
elimination (\ref{DS}) generalizes to non-relaxed access by generalizing
Definition \ref{def:cover} of $\relfilt[]{\aLoc}$.  Some optimizations are
not sound in the model, but expect them to be provable as meta properties.
For example, access-mode strengthening (such as replacing $\modeRLX$ by
$\modeRA$) is valid up to an equivalence that ignores access modes on
actions.  Similar reasoning can likely be used to show that non-relaxed
access can be implemented using relaxed access and fences.  We expect that
similar techniques can be developed for optimizations that are only sound in
certain contexts, such as lock elision and access-mode weakening.
...
\end{verbatim}
  I am worried about the compositionality of this predicate:
\begin{verbatim}
I think
   <>(W x 0 => not(<>(W x 1)))
holds for 
...
it disallows \ref{MCA1}\footnote{Following \cite{DBLP:journals/dc/Lamport86},
  3-valued pomset require: (1) if $d\leq e$ then $d\gtNsp e$, (2) if
  $d\leq e$ and $e\gtNsp d$ then $d=e$, and (3) if $c\leq d \gtNsp e$ or
  $c\gtNsp d\leq e$ then $c\gtNsp e$.  \ref{MCA1} is disallowed by (2).
  ...
  then $d$ and $e$ do not touch the same location.  Although \ref{MCA2} is
