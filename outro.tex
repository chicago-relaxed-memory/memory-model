...
our semantics validates roach motel (\ref{RelW}, \ref{AcqW}), redundant load (\ref{RL}),
fence removal (\ref{RF}), and store forwarding (\ref{SF}).  We
expect that dead store elimination (\ref{DS}) generalizes to non-relaxed
access by generalizing the Definition \ref{def:cover} of $\relfilt[]{\aLoc}$.
...
address this example; however, \weakestmo{} does not address \ref{OOTA1}.  
