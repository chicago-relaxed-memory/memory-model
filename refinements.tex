...
\begin{definition}
  ...
  \end{align*}
\end{definition}
There are five changes in the definition: To validate read elimination, we
...
analysis in the absence of read elimination, we could either strengthen
\ref{5b} to require read-read coherence or apply $\Rdis{\aLoc}{\aVal}$ to
$\sem{\aCmd} [\aLoc/\aReg]$ before prefixing; we chose the latter since it is
robust with respect to read elimination.  To validate write elimination, we
include $\relfilt[]{\aLoc} \sem{\aCmd}[\aExp/\aLoc]$. To validate case
analysis with write elimination, we apply $\Wdis{\aLoc}{\aVal}$.
...
% analysis \eqref{CA}:
Case analysis \eqref{CA} allows code to be optimized for common special cases:
\begin{align*}
  \taglabel{CA}
  \sem{\aCmd} &\supseteq
  \sem{\IF{\aExp}\THEN\aCmd\ELSE\aCmd\FI} 
\end{align*}
\ref{CA} fails for Candidate \ref{cand:ord} due to the lack of read-read coherence.  Consider  commands $\cmdR$ and $\cmdW$:
\begin{align*}
  \begin{gathered}
    \cmdR = s\GETS x \SEMI \IF{r}\THEN s\GETS x \FI\\
    \hbox{\begin{tikzinline}[node distance=1em]
        \event{a}{\DR{x}{0}}{}
        \event{b}{r\mid\DR{x}{0}}{right=of a}
      \end{tikzinline}}
  \end{gathered}
  &&
  \begin{gathered}
    ...
    \hbox{\begin{tikzinline}[node distance=1em]
        \event{a}{\DW{x}{1}}{}
        \event{b}{r\mid\DW{x}{1}}{right=of a}
        \wk{a}{b}
      \end{tikzinline}}
  \end{gathered}
\end{align*}
...
\begin{align*}
  \begin{gathered}
    ...
    \\
    \hbox{\begin{tikzinline}[node distance=1em]
        \event{a}{s \mid\DR{x}{0}}{}
        \event{b}{s \land r\mid\DR{x}{0}}{right=of a}
      \end{tikzinline}}
  \end{gathered}
  &&
  \begin{gathered}
    ...
    \\
    \hbox{\begin{tikzinline}[node distance=1em]
        \event{a}{\lnot s \mid\DR{x}{0}}{}
        \event{b}{\lnot s \land r\mid\DR{x}{0}}{right=of a}
      \end{tikzinline}}
  \end{gathered}
\end{align*}
...
\begin{align*}
  \begin{gathered}
    ...
    \\
    \hbox{\begin{tikzinline}[node distance=1em]
        \event{a}{s\lor (\lnot s \land r) \mid\DR{x}{0}}{}
        \event{b}{(s \land r)\lor \lnot s \mid\DR{x}{0}}{right=of a}
      \end{tikzinline}}    
  \end{gathered}
\end{align*}
which is:
\begin{align*}
  \begin{gathered}
    \hbox{\begin{tikzinline}[node distance=1em]
        \event{a}{s\lor r\mid\DR{x}{0}}{}
        \event{b}{\lnot s\lor r\mid\DR{x}{0}}{right=of a}
      \end{tikzinline}}
  \end{gathered}
\end{align*}
...
Without read or write elimination, \ref{CA} does hold if one is willing to
strengthen \ref{5b} to include read-read coherence and, thus, sacrifice
\ref{CSE} (\textsection\ref{sec:valid}).  This compromise is considered
reasonable for C11 atomics, which are meant to be used sparingly.  It is less
attractive for plain access in Java-like languages.  However, \ref{CA} fails
for the semantics with read elimination, even with read-read coherence.  For
example, consider the following executions of $\cmdR$:
\begin{align*}
  \hbox{\begin{tikzinline}[node distance=1em]
      \event{a}{\DR{x}{0}}{}
      \event{b}{r\mid\DR{x}{0}}{right=of a}
...
$\IF{s}\THEN \cmdR \ELSE \cmdR \FI$ to include $(\TRUE\mid\DR{x}{0})$, but it
does not under Candidate \ref{cand:ord}.  
Our solution is to weaken preconditions on identical reads when prefixing; thus
$\sem{\cmdR}$ contains:
\begin{gather*}
  \hbox{\begin{tikzinline}[node distance=1em]
      \event{a}{\DR{x}{0}}{}
      \event{b}{\DR{x}{0}}{right=of a}
    \end{tikzinline}}
\end{gather*}
...
\begin{align*}
  ...
  &&
  \hbox{\begin{tikzinline}[node distance=1em]
      \event{b}{r\mid\DW{x}{1}}{}
    \end{tikzinline}}
\end{align*}
Taking the disjunction of their downsets, we expect the semantics of
$\IF{s}\THEN \cmdW \ELSE \cmdW \FI$ to include $(\TRUE\mid\DW{x}{1})$, but it
does not under Candidate \ref{cand:ord}.  As before, our solution is to
weaken the preconditions on identical writes when prefixing.  We do this
using the function $\Wdis{\aLoc}{\aExp}$, which weakens the precondition of
any $<$-minimal $\DW[\modeRLXquiet]{\aLoc}{\aVal}$ to
...
arrive at the formula $(1=1\lor r)$ for the ``old'' write, which is a tautology.
...
The key property satisfied by Definition \ref{def:cover} is \emph{disjunction
  closure}.
\begin{definition}
  \label{def:dis}
  ...
  $\labelingAct^0=\labelingAct^1 \supseteq\labelingAct^2$, 
  $\labelingForm^0(\aEv)$ implies
  $\labelingForm^1(\aEv)\lor \labelingForm^2(\aEv)$ if $\aEv\in\Event^2$, and
  $\labelingForm^0(\aEv)$ implies
  $\labelingForm^1(\aEv)$ otherwise.
  % and:
  % \begin{displaymath}
