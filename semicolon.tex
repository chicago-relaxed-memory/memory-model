...
The following definition include events from pomsets in $\aPSS^2$  in
$\aPSS^1 \sequence \aPSS^2$ only when the pomset chosen from $\aPSS^1$ has an
accepting state.  Since $\DISJUNCT$ does not perform prefix closure, we
include $\aPSS^1$ in $\aPSS^1 \sequence \aPSS^2$ separately.
