...
includes $\aPSS$ and that is closed w.r.t.~disjunction and prefix weakening.
...
\textsection\ref{sec:model}.  Item 5 is morally unchanged. 
 Items 4 and 6 are more complicated here:
...
rule.  The requirement for a final write is imposed at top level---see below.
...
Let $\coverEnd\aPSS$ be the set $\aPSS'$, where $\aPS'\in\aPSS'$ when for
every $\labelingAct'(\bEv)=\DFW[\aLoc]{\amode}$ there is some $\aEv\gt\bEv$
that writes $\aLoc$.
...
\begin{math}
  \semold{\aCmd} = \{ \aPS\in\coverEnd\sem{\aCmd} \mid \disjoint{\Event}{\Sub}\}.
\end{math}
% \begin{align*}
