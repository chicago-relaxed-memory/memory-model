...
$\sem{\bCmd}$.  To simplify the definitions, we assume, without loss of
generality, that each register is
...
Definition~\ref{def:mmpomset} of \emph{memory model pomset} changes so that
$\labeling: \Event \fun (\Formulae\times\Act\times\Sub)$, from which we
derive the additional function $\labelingSub:\Event\fun\Sub$.

In examples, we write the substitution as a subscript of the action on an
event.  We elide the identity substitution.
...
Definition \ref{def:closure:properties} of relations between pomsets is
extended to include subsumption: $\aPS'$ \emph{subsumes} $\aPS$ if
...
Subsumption is dual to implication:
...
$\afterSub$ such that
$\labelingSub(\aEv)=\beforeSub\SUBSEQ\SUB{\aLoc/\aReg}\SUBSEQ\afterSub$ and
$(\beforeSub\SUBSEQ\afterSub) = \labelingSub(\aPS)$.

We also update the the partial function $\rreads$  from the data model so
that $\rreads:\Act\fun(\Reg \times \Loc \times \Val)$.
When $\rreads(\aAct) = (\aReg,\aLoc,\aVal)$, we say that $\aAct$ \emph{reads}
...
Definition \ref{def:rf} of \emph{$\aLoc$-closed} changes to require that
every substitution is independent of $\aLoc$.

Definition \ref{def:par} of $\aPS' \in (\aPSS^1 \parallel \aPSS^2)$ changes
...
We draw termination events as accepting states, dropping the termination action $(\DSTOP)$.
% : Let
...
\begin{enumerate}
\item[1.] $\Event' = \Event^2 \uplus \{\bEv\in\Event^1\mid\labelingAct(\bEv)\neq\DSTOP\}$,
...
\item[4bc.] $\labelingForm'(\aEv)$ implies
  $(\SUBDRS{\aPS^1} \SUBSEQ \labelingSub(\aPS^1))\SUBAPP\labelingForm^2(\aEv)$, 
\item[5a.] if $\bEv$ is a read and $\aEv$ is a write,
  then either $\bEv\le'\aEv$ or $\labelingForm'(\aEv)$ implies $(\SUBDRS{\aPS^1}\SUBSEQ \labelingSub^1(\bEv))\SUBAPP\labelingForm^2(\aEv)$,
...
\textsection\ref{sec:model}.  Item 5 is morally unchanged.  
Items 4b and 4c collapse into a single item here.

In item 4bc, note that the domain of $\labelingSub(\aPS^1)$ is disjoint from the domain
...
In item 5a, recall that $\labelingSub^1(\bEv)=\beforeSub\SUBSEQ\SUB{\aLoc/\aReg}\SUBSEQ\afterSub$ and
