...
substitutions as silent actions: $\Sub\subseteq\Int\subseteq\Act$.  In the context of a
...
% substitution:
Our pomsets will contain at most one substitution event, which is ordered
after every other event.  In examples, we typically drop order into
...
$\aReg$.
For any $\aAct$, we say that $\aAct$ \emph{read-substitutes} $\bSub$ if either
$\aAct=(\DRreg[\amode]{\aReg}{\bSub}{\aLoc}{\aVal})$ or $\aAct$ is not a read
and $\bSub=\SUBEMP$.

We modify Definition \ref{def:rf} to require that when $\aPS$ is
\emph{$\aLoc$-closed}, every substitution in $\aPS$ is the identity on
$\aLoc$: If $\labelingAct(\aEv)=\aSub$ or $\aEv$ read-substitutes $\aSub$,
then $\aSub\SUBAPP\aLoc=\aLoc$.
...
or (3) $\aAct$ read-substitutes $\aSub$,
$\bAct$ read-substitutes $\bSub$, and $\aSub$ subsumes
$\bSub$.
...
Subsumption-maximal pomsets that include an accepting state
$\aSub=\labelingAct(\Event\cap\Sub)$ satisfy the invariant that when
$\labelingAct(\aEv)=(\DRreg[\amode]{\aReg}{\bSub}{\aLoc}{\aVal})$ there are
...
We elide the definitions of conditional and location binding, which can be
imported directly from \textsection\ref{sec:model}---using the modified
definition of $\aLoc$-closure discussed above.
...
Let $\killS\aPSS$ be the set $\aPSS'\subseteq\aPSS$ where $\aPS'\in\aPSS'$ when
$\disjoint{\Event'}{\Sub}$ and every $\aEv'\in\Event'$ read-substitutes $\SUBEMP$.
%
\begingroup
\allowdisplaybreaks
\begin{gather*}
...
\begin{math}
  (\SKIP\PAR r\GETS 1)\SEMI x\GETS r
\end{math}
cannot.
...
\item[5g.] if $\aEv\in\Sub$ then $\dEv\le'\aEv$,
...
\begin{proposition}
  ...
  extended with silent actions and fences, adopting the read actions of this
  section---with the read substitution being $\SUBEMP$.
\begin{displaymath}
