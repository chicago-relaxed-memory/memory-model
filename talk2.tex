...
\begin{frame}
  ...
  \end{displaymath}
  \only<2>{\sol{Merging!}}
  \onslide<5->{\prob{Temporal Reasoning?}}
  \onslide<6->{\sol{Temporal Reasoning!}}
\end{frame}
...
\begin{frame}
  ...
  \begin{gather*}
    ...
    \PW{b}{1}
    \\
    \onslide<2->{\hbox{\begin{tikzinline}[node distance=1.5em]
        \event{rx}{\DR{x}{1}}{}
  ...
  \normalsize
  \onslide<3->{Needed for invariant reasoning, e.g. type safety!\\
    (Example is simplified from Lohbieler's type unsafety for JMM.)}
